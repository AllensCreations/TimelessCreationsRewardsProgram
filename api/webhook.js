...
async function callSendAPI(senderPsid, responseText, quickReplies = null) {
  ...
  try {
    ...
    if (!res.ok) {
      console.error("❌ Meta Send API Error:", JSON.stringify(data));
    }
  } catch (err) {
...
async function sendButtonMessage(senderPsid, text, buttons) {
  ...
  try {
    ...
    const data = await res.json();
    if (!res.ok) {
      console.error("❌ Meta Button API Error:", JSON.stringify(data));
    }
  } catch (err) {
...
// 6. PRODUCT CATALOG CAROUSEL TEMPLATE
async function sendCatalogCarousel(senderPsid) {
  ...
  try {
    ...
    const data = await res.json();
    if (!res.ok) {
      console.error("❌ Meta Carousel API Error:", JSON.stringify(data));
    }
  } catch (err) {
...
// 8. 5-IN-1 FAQS BUILDER
function getFaqsText() {
  return `❓ 𝐅𝐑𝐄𝐐𝐔𝐄𝐍𝐓𝐋𝐘 𝐀𝐒𝐊𝐄𝐃 𝐐𝐔𝐄𝐒𝐓𝐈𝐎𝐍𝐒\n` +
    `━━━━━━━━━━━━━━━━━━━━━━\n\n` +
    `𝟏. 𝐖𝐡𝐚𝐭 𝐢𝐬 𝐓𝐂𝐑𝐏?\n` +
    `An exclusive missionary rewards program by Timeless Creations offering custom missionary gear.\n\n` +
          ...
          try {
            ...
            if (messageText === "PAYLOAD_TERMS_INFO" || messageText.toLowerCase() === "terms") {
              const termsInfo = `📜 𝐓𝐄𝐑𝐌𝐒 & 𝐂𝐎𝐍𝐃𝐈𝐓𝐈𝐎𝐍𝐒\n━━━━━━━━━━━━━━━━━━━━━━\n` +
                ...
                `• Rewards are subject to identity and order verification.\n` +
                `• Timeless Creations reserves the right to manage program rules.`;
