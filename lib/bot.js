...
export async function checkDashboardRateLimit(senderId) {
  ...
  try {
    ...
    const isWarned = Number(rows?.[0]?.warned ?? 0);

    if (count >= 2) {
      if (isWarned === 1) {
        return { allowed: false, remaining: 0, shouldMute: true, message: limitMsg };
      }
      ...
      return { allowed: false, remaining: 0, shouldMute: false, message: limitMsg };
    }

    // Increment view count
    await runSql(
      "UPDATE bot_daily_views SET view_count = view_count + 1 WHERE sender_id = ? AND view_date = ?",
      [senderId, todayStr]
    );
    ...
    return { allowed: true, remaining: 1, shouldMute: false, message: defaultLimitMsg };
  }
}
