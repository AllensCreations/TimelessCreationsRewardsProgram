...
export async function checkDashboardRateLimit(senderId) {
  try {
    ...
    // Fetch current state
    const currentRows = await runSql(
      "SELECT view_count, warned FROM bot_daily_views WHERE sender_id = ? AND view_date = ? LIMIT 1",
      [senderId, todayStr]
    );
    const viewCount = Number(currentRows?.[0]?.view_count || 0);
    const isWarned = Number(currentRows?.[0]?.warned || 0);

    if (viewCount >= 2) {
      if (isWarned === 1) {
        return { allowed: false, shouldMute: true, message: '' };
      }
