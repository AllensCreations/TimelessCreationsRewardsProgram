...
export async function checkDashboardRateLimit(senderId) {
  ...
  let currentCount = memRateLimit.get(cacheKey) || 0;
  ...
  try {
    ...
    if (dbRows && dbRows.length > 0) {
      currentCount = Math.max(currentCount, Number(dbRows[0]?.view_count || 0));
    }
  } catch (e) {
    console.error("DB rate limit read error:", e.message);
  }

  if (currentCount >= 2) {
    memRateLimit.set(cacheKey, currentCount + 1);
    try {
      await runSql(
    ...
    return { 
      allowed: false, 
      remaining: 0, 
      shouldMute: currentCount > 2, 
      message: limitMsg 
    };
  }

  const newCount = currentCount + 1;
  memRateLimit.set(cacheKey, newCount);
  ...
  try {
    ...
    console.error("DB rate limit write error:", e.message);
  }
...
// Separated Dashboard and Invite Payloads
...
// 1:1 Square Image Carousel showing strictly Name and Cost
          ...
          elements: [
            {
              ...
              subtitle: `⭐ Cost: 0 PTS | Current Balance: ${senderPoints} PTS`,
