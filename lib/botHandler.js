...
export async function handleBotMessage(psid, rawMessage = '', payload = null, referralParam = '') {
  ...
  // 1. Unified Action Trigger Resolution (Matches either payload OR quick reply text)
  ...
  // 2. GLOBAL COMMAND: /delete_account
  ...
  // 4. ONBOARDING QUICK REPLIES (Processed even if session is unlinked)
  if (isAgreeAction) {
...
${defaultCode}`;
  ...
  // 5. VERIFIED USER ACTIONS & CAROUSELS
  if (isVerified) {
    // Promo Code Command (/redeem or /rewards)
  ...
  // 6. UNVERIFIED ONBOARDING STATE MACHINE
