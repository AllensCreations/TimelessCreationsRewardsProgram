...
// Structured Policy & Information Texts
...
export async function handleBotMessage(psid, rawMessage, payload = null) {
  ...
  // Persistent Menu Trigger: DISCOVER / DASHBOARD FLOW
  if (lower === 'discover' || lower === 'dashboard' || payload === 'DISCOVER_PAYLOAD' || payload === 'CATALOG_PAYLOAD') {
    ...
    if (missionary) {
      ...
      let notifBanner = "";
      if (totalReferred > 0) {
        notifBanner = `\n🔔 REFERRAL UPDATE: ${totalReferred} missionary companion(s) joined using your code!\n`;
      }
