...
export async function handleBotMessage(psid, rawMessage, payload = null) {
  ...
  if (!isVerified) {
    ...
    if (session.state === 'AWAITING_OTP') {
      // Handle Quick Reply actions FIRST before checking digit validation
      if (payload === 'RESEND_OTP' || lower.includes('resend') || lower.includes('change name')) {
        if (payload === 'REENTER_INFO' || lower.includes('change name')) {
          await runSql("UPDATE sessions SET state = 'AWAITING_NAME_EMAIL' WHERE psid = ?", [psid]);
