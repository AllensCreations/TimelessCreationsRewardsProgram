...
async function renderDashboardAndCarousel(psid, prefixMessage = "") {
  ...
  // STRICT AUTH GUARD: Prevent unverified/guest users from bypassing to dashboard
  if (!missionary) {
    await runSql("UPDATE sessions SET state = 'AWAITING_REFERRAL' WHERE psid = ?", [psid]);
    await logSystemEvent('TURSO', `[SECURITY BLOCK] Unverified PSID ${psid} attempted dashboard bypass. Redirecting to verification.`);
