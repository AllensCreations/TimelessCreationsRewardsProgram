...
export async function handleBotMessage(psid, rawMessage = '', payload = null, referralParam = '') {
  ...
  // Sanitize referral params to ignore emoji menu titles like "📊 DASHBOARD"
  const cleanRef = (rawRef.includes('DASHBOARD') || rawRef.includes('FAQS') || rawRef.includes('REWARDS') || rawRef.includes('PROMO') || rawRef.length > 10) ? '' : rawRef;
  ...
  if (session.state === 'AWAITING_REFERRAL') {
    ...
    // If user clicked persistent menu button while unverified
    if (inputCode.includes('DASHBOARD') || inputCode.includes('MENU')) {
      await sendTextMessage(
        psid,
        "⚠️ You must complete verification first before accessing your dashboard!\n\nPlease enter your Referral Code (e.g. TCRP50):",
        [{ title: "✨ Get Started", payload: "GET_STARTED" }]
      );
      return;
    }
