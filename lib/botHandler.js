...
// Anti-Spam Rate Limiter: Max 10 messages per 60 seconds
async function checkAntiSpam(psid) {
  ...
  try {
    ...
    if (record.msg_count >= MAX_MSGS) {
      const shouldWarn = (now - (record.warned_at || 0)) > 30;
      if (shouldWarn) {
        await runSql("UPDATE bot_rate_limits SET warned_at = ? WHERE psid = ?", [now, psid]);
      }
...
💡 Tap 'Get Started' below to enter your referral code and verify your official @missionary.org email!`;
  ...
  const userPts = missionary ? (Number(missionary.points) || 0) : 0;
  await sendProductCarousel(psid, products, userPts);
}

async function getDynamicFaqText() {
  let rewardList = "";
  try {
    const products = await runSql("SELECT name, CAST(price AS INTEGER) as price FROM product_catalog WHERE type = 'reward' ORDER BY price ASC");
    if (products && products.length > 0) {
      rewardList = products.map(p => `• ${p.name}: ${p.price} Points`).join("\n");
    }
  } catch (_) {}

  if (!rewardList) {
    rewardList = "• Temple Keychain: 6 Points\n• Nametag Keychain: 24 Points\n• Salvation Kit: 42 Points\n• Scripture Case: 60 Points";
  }
...
export async function handleBotMessage(psid, rawMessage, payload = null) {
  ...
  if (!spamCheck.allowed) {
    if (spamCheck.shouldWarn) {
      await sendTextWithQuickReplies(
        psid,
        "⏳ Rate Limit Exceeded: You are sending messages too quickly. Please wait a minute before trying again."
      );
    }
    return;
  }
  ...
  if (payload && (payload.startsWith('REDEEM_ITEM_') || payload.startsWith('REDEEM_'))) {
    ...
    if (!product) {
      ...
      await sendProductCarousel(psid, activeCatalog, Number(missionary?.points) || 0);
      return;
    }
...
Staff will prepare your custom item. Please present Reference ID #${refId} in this chat for custom engraving details & dispatch!`;
  ...
  if (lower === 'faqs' || payload === 'FAQS_PAYLOAD') {
    const faqMsg = await getDynamicFaqText();
    await sendTextWithQuickReplies(psid, faqMsg, [{ title: "🛍️ Discover", payload: "DISCOVER_PAYLOAD" }]);
    return;
  }

  if (lower === 'discover' || lower === 'dashboard' || payload === 'DISCOVER_PAYLOAD' || payload === 'CATALOG_PAYLOAD' || payload === 'ACTION_DASHBOARD') {
