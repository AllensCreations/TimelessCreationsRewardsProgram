...
export async function handleBotMessage(psid, rawMessage = '', payload = null, referralParam = '') {
  ...
  if (isVerified) {
    ...
    if (!viewLimit.allowed) {
      const currentPoints = Number(currentM?.points ?? missionary.points) || 0;
      const noticePrefix = refNotice ? `${refNotice}\n\n` : "";
      await sendTextMessage(
        psid,
        `${noticePrefix}You have already checked your rewards dashboard today.\n\nTo ensure fair access and dependable service, the full rewards dashboard and catalog can only be checked once per day.\n\n(Note: We will not reply as of the moment. The next Check will be tomorrow at 8:00 AM PHT.)\n\nCurrent Balance: ${currentPoints} Reward Point(s)\n\nThank you for your understanding!`,
        SINGLE_QUICK_REPLY
      );
      return;
    }
