...
/**
 * Hourly Verified Dashboard Rate Limiter (PST / UTC+8)
...
Answer: Tap '🎁 Claim' in the Rewards Catalog carousel to generate an official order receipt.
...
export async function handleBotMessage(psid, rawMessage = '', payload = null, referralParam = '') {
  ...
  // Normalization for Quick Reply & Postback Action Triggering
  const isAgreeAction = cleanPayload === 'TERMS_AGREE' || lower === 'agree & continue' || (lower.includes('agree') && lower.includes('continue'));
  const isDeclineAction = cleanPayload === 'TERMS_DECLINE' || lower === 'decline' || lower.includes('decline');
  const isGetStartedAction = cleanPayload === 'GET_STARTED' || lower === 'get started' || lower === 'start';
  const isFaqsAction = cleanPayload === 'FAQS_PAYLOAD' || cleanPayload === 'MENU_FAQS' || lower === 'faqs' || lower === 'faq' || lower.includes('about tcrp');
  const isDiscoverAction = cleanPayload === 'DISCOVER_PAYLOAD' || cleanPayload === 'MENU_REWARDS' || lower === 'discover' || lower === 'rewards' || lower === 'catalog';
  const isPromoInfoAction = cleanPayload === 'PROMO_INFO' || lower === 'promo' || lower === 'promo code';
  const isDashboardAction = cleanPayload === 'ACTION_DASHBOARD' || lower === 'dashboard' || lower === 'menu';
  const isResendOtpAction = cleanPayload === 'RESEND_OTP' || lower === 'resend code';

  // 1. Account Deletion Command
  ...
  // 3. STEP 1: TERMS & PRIVACY ACCEPTANCE
  if (isAgreeAction) {
    await runSql(`
      INSERT INTO sessions (psid, state, invite_code)
      VALUES (?, 'AWAITING_ALL_IN_ONE', ?)
      ON CONFLICT(psid) DO UPDATE SET state = 'AWAITING_ALL_IN_ONE', invite_code = COALESCE(excluded.invite_code, sessions.invite_code)
    `, [psid, cleanRef || 'TCRP50']);

    const session = (await runSql("SELECT invite_code FROM sessions WHERE psid = ? LIMIT 1", [psid]))?.[0];
    const defaultCode = session?.invite_code || (cleanRef || "TCRP50");
    const prompt = 
`✅ 𝗧𝗲𝗿𝗺𝘀 𝗔𝗰𝗰𝗲𝗽𝘁𝗲𝗱!
...
${defaultCode}`;
  ...
  if (isResendOtpAction) {
    const session = (await runSql("SELECT * FROM sessions WHERE psid = ? LIMIT 1", [psid]))?.[0];
    ...
    await sendTextMessage(psid, `🔄 A fresh 6-digit code has been sent to ${session?.temp_email || 'your email'}. Please type it below:`, [
      { title: "✏️ Edit Info", payload: "GET_STARTED" }
    ]);
    return;
  }

  // 4. VERIFIED MISSIONARY DASHBOARD & ACTIONS
  ...
  if (isVerified) {
    ...
    if (cleanPayload.startsWith("CLAIM_ITEM_")) {
      ...
      await sendTextMessage(
        psid,
        `🎉 𝗥𝗘𝗗𝗘𝗠𝗣𝗧𝗜𝗢𝗡 𝗖𝗢𝗡𝗙𝗜𝗥𝗠𝗘𝗗!\n\n📦 𝗜𝘁𝗲𝗺: ${prod.name}\n⭐ 𝗣𝗼𝗶𝗻𝘁𝘀 𝗨𝘀𝗲𝗱: ${cost} PTS\n🆔 𝗢𝗿𝗱𝗲𝗿 𝗜𝗗: ${orderId}\n\nYour order has been recorded and an official receipt was sent to ${missionary.email}.`,
        [
          { title: "📊 Dashboard", payload: "ACTION_DASHBOARD" },
          { title: "🛍️ Discover", payload: "DISCOVER_PAYLOAD" }
        ]
    ...
    if (cleanPayload.startsWith("GOAL_ITEM_")) {
      ...
      if (prod) {
        ...
        await sendTextMessage(
          psid,
          `🎯 𝗡𝗘𝗫𝗧 𝗚𝗢𝗔𝗟: ${prod.name}\n\n⚡ You only need ${needed} more point${needed > 1 ? 's' : ''} to unlock this reward!\n\nShare your link:\n${refLink}`,
          [
            { title: "📊 Dashboard", payload: "ACTION_DASHBOARD" },
            { title: "🛍️ Discover", payload: "DISCOVER_PAYLOAD" }
          ]
  ...
  // 5. UNVERIFIED ONBOARDING FLOW
  let session = (await runSql("SELECT * FROM sessions WHERE psid = ? LIMIT 1", [psid]))?.[0] || null;

  if (!session) {
    await runSql("INSERT INTO sessions (psid, state, invite_code) VALUES (?, 'AWAITING_TERMS', ?)", [psid, cleanRef || 'TCRP50']);
    await sendWelcomeAndTerms(psid);
    return;
  }

  if (isGetStartedAction) {
  ...
  if (session.state === 'AWAITING_OTP') {
    ...
    if (cleanedCode === session.otp_code) {
      ...
      // Automatically inserts verified Title & Email into missionaries table
      await runSql(`
