...
import { getFirstMonthInfo, calculateMissionMonth, getFirstDispatchDate } from './utils/batchCalculator.js';
...
const inFlightRedemptions = new Set();
const inFlightPromos = new Set();
...
async function sendFbGraphMessage(psid, messagePayload) {
  try {
    ...
    await runSql("INSERT INTO system_logs (source, psid, sender, message) VALUES ('CHAT_MESSENGER', ?, 'bot', ?)", [psid, textPreview]);
  } catch (_) {}
  ...
  if (!PAGE_ACCESS_TOKEN || PAGE_ACCESS_TOKEN.startsWith('EAA_MOCK') || isMockPsid) {
    const raw = messagePayload.text || (messagePayload.attachment ? '[Carousel / Template]' : '[Message]');
    const preview = Array.from(raw).slice(0, 80).join('').replace(/[\r\n]+/g, ' ');
...
/**
 * Returns quick reply buttons for verified missionaries.
 * If their 1 daily check limit has been consumed for today, the "Check" quick reply
 * is temporarily hidden until 8:00 AM PHT the next day!
 */
export async function getVerifiedQuickReplies(psid) {
  try {
    const checkUsed = await hasUsedDailyCheck(psid);
    if (checkUsed) {
      return []; // Temporarily hidden until 8:00 AM PHT!
    }
  } catch (_) {}
  return SINGLE_QUICK_REPLY;
}
...
export async function sendCatalogCarouselOrNote(psid, points = 0) {
  ...
  if (carousel) {
    const checkUsed = await hasUsedDailyCheck(psid);
    const payloadToSend = checkUsed
      ? { ...carousel, quick_replies: undefined }
      : carousel;
    await sendFbGraphMessage(psid, payloadToSend);
  }
}
...
export async function sendDashboardMessage(psid, missionary, prefixMsg = "") {
  ...
  const points = Number(currentM?.points ?? missionary?.points) || 0;
...
export async function sendInviteLink(psid, missionary) {
  ...
  const quickReplies = await getVerifiedQuickReplies(psid);
  await sendTextMessage(psid, invitePromoText, quickReplies.length > 0 ? quickReplies : undefined);
}

export async function sendVerifiedHub(psid, missionary, prefixMsg = "") {
  const currentM = (await runSql("SELECT points, pending_ref_notices FROM missionaries WHERE psid = ? LIMIT 1", [psid]))?.[0];
  const points = Number(currentM?.points ?? missionary?.points) || 0;

  // Check and consume pending referral notifications
  let refNotice = "";
  const pendingNotices = Number(currentM?.pending_ref_notices || missionary?.pending_ref_notices || 0);
  if (pendingNotices > 0) {
    const celebrationTitle = toUnicodeBold("🎉 REFERRAL REWARD RECEIVED!");
    const countNotice = pendingNotices === 1 
      ? "Someone used your referral code (+1 Point added to your balance)!"
      : `${pendingNotices} missionaries used your referral code (+${pendingNotices} Points added to your balance)!`;
    refNotice = `${celebrationTitle}\n${countNotice}`;
    await runSql("UPDATE missionaries SET pending_ref_notices = 0 WHERE psid = ?", [psid]);
  }

  const combinedPrefix = [prefixMsg, refNotice].filter(Boolean).join('\n\n');

  // 1. Dashboard message
  await sendDashboardMessage(psid, missionary, combinedPrefix);
...
export async function sendHelpAndFaqs(psid, isVerified = false) {
  ...
  const verifiedQr = isVerified ? (await getVerifiedQuickReplies(psid)) : null;
  const quickReplies = isVerified 
    ? (verifiedQr && verifiedQr.length > 0 ? verifiedQr : undefined) 
    : [
        { title: "Verify & Join (+1)", payload: "GET_STARTED" },
...
export async function handlePromoRedeem(psid, missionary, codeText) {
  ...
  const vQr = await getVerifiedQuickReplies(psid);
  const quickReplies = vQr && vQr.length > 0 ? vQr : undefined;

  if (!code) {
    await sendTextMessage(psid, "Please specify a promo code.\n\nExample:\n/redeem SPECIALGIFT", quickReplies);
    return;
  }

  const mEmail = (missionary?.email || '').toLowerCase().trim();
  const lockKey = `${psid}_${code}`;
  if (inFlightPromos.has(lockKey)) {
    return;
  }
  inFlightPromos.add(lockKey);
  ...
  try {
    ...
    if (!promo) {
      await sendTextMessage(psid, `Promo code "${code}" is invalid or has expired.`, quickReplies);
      return;
    }

    if (Number(promo.claimed_count) >= Number(promo.max_users)) {
      await sendTextMessage(psid, `Promo code "${code}" has reached its maximum user claim limit (${promo.max_users}/${promo.max_users}).`, quickReplies);
      return;
    }

    const alreadyClaimed = await runSql(
      "SELECT 1 FROM promo_redemptions WHERE UPPER(code) = ? AND (psid = ? OR (email IS NOT NULL AND LOWER(email) = LOWER(?))) LIMIT 1",
      [code, psid, mEmail]
    );
    if (alreadyClaimed && alreadyClaimed.length > 0) {
      await sendTextMessage(psid, `You have already redeemed promo code "${code}".`, quickReplies);
      return;
    }
    ...
    await runSql("INSERT OR IGNORE INTO promo_redemptions (code, psid, email) VALUES (?, ?, ?)", [code, psid, mEmail]);
    await runSql("UPDATE missionaries SET points = points + ? WHERE psid = ?", [bonusPoints, psid]);

    await logEvent('INFO', `Promo ${code} (+${bonusPoints} PTS) redeemed by ${missionary?.name || 'Missionary'} (${mEmail || psid})`, psid);
    ...
    await sendTextMessage(
      psid,
      `${congratsHeader}\n\nYou successfully claimed promo code ${code}!\n+${bonusPoints} Reward Point(s) added to your balance.\n\nNew Balance: ${updatedMissionary ? updatedMissionary.points : bonusPoints} Points.`,
      quickReplies
    );
  } finally {
    inFlightPromos.delete(lockKey);
  }
}

export function parseMissionaryRegistrationDetails(text, fallbackRef = null) {
  if (!text || typeof text !== 'string') return null;

  // 1. Email check: must contain a valid @missionary.org email
  const emailMatch = text.match(/([a-zA-Z0-9._%+-]+@missionary\.org)/i);
  if (!emailMatch) return null;
  const emailInput = emailMatch[1].toLowerCase();

  // 2. Month Batch check: must match a valid month
  ...
  if (lines.length >= 2) {
    const candidateLines = lines.filter(l => !l.toLowerCase().includes('@missionary.org') && !monthRegex.test(l));
    for (const line of candidateLines) {
      const upper = line.trim().toUpperCase();
      const isRef = /^[A-Z0-9]{4,12}$/i.test(upper) && !/^(elder|sister)$/i.test(upper) && (upper.length <= 8 || /\d/.test(upper));
      if (!referralCode && isRef && titleName) {
        referralCode = upper;
      } else if (!titleName) {
        titleName = line.trim();
      } else if (!referralCode && isRef) {
        referralCode = upper;
      }
    }
  } else {
    let remaining = text.replace(emailMatch[0], '').replace(monthMatch[0], '').trim();
    const parts = remaining.split(/\s+/).filter(Boolean);
    if (parts.length >= 2) {
      const last = parts[parts.length - 1];
      const isRef = /^[A-Z0-9]{4,12}$/i.test(last) && !/^(elder|sister)$/i.test(last) && (last.length <= 8 || /\d/.test(last));
      if (isRef) {
        referralCode = last.toUpperCase();
        titleName = parts.slice(0, -1).join(' ').replace(/^[,\s]+|[,\s]+$/g, '').trim();
      } else {
        titleName = remaining.replace(/^[,\s]+|[,\s]+$/g, '').replace(/,/g, ' ').replace(/\s+/g, ' ').trim();
      }
    } else if (parts.length === 1) {
      titleName = parts[0].replace(/^[,\s]+|[,\s]+$/g, '').trim();
    }
  }

  // Automatic referral fallback from session invite link if not explicitly provided in message
  if (!referralCode && fallbackRef && typeof fallbackRef === 'string' && fallbackRef.trim().length >= 4) {
    referralCode = fallbackRef.trim().toUpperCase();
  }

  // ALL FOUR INFORMATIONS ARE STRICTLY NEEDED
  if (!emailInput || !titleName || !batchMonth || !referralCode) {
    return null;
  }

  if (titleName.length < 2 || titleName.toUpperCase() === referralCode.toUpperCase()) {
    return null;
  }

  // Strictly require explicit 'Elder' or 'Sister' title per design
  const parsedName = parseMissionaryName(titleName);
  if (!parsedName.valid && !/^(elder|sister)\b/i.test(titleName)) {
    return null;
  }

  const resolvedTitle = parsedName.valid ? parsedName.name : titleName;

  return { emailInput, titleName: resolvedTitle, batchMonth, referralCode };
}
...
export async function handleBotMessage(psid, rawMessage = '', payload = null, referralParam = '') {
  ...
  try {
    await runSql("INSERT INTO system_logs (source, psid, sender, message) VALUES ('CHAT_MESSENGER', ?, 'user', ?)", [psid, text || cleanPayload || cleanRef || '[Action]']);
  } catch (_) {}
  ...
  if (lower === 'reset' || cleanPayload === 'RESET_SESSION' || lower === 'restart') {
    ...
    await runSql("DELETE FROM bot_limits WHERE psid = ?", [psid]);
  ...
  if (lower === '/delete_account' || lower === 'delete account') {
    ...
    await runSql("DELETE FROM sessions WHERE psid = ?", [psid]);
    await runSql("DELETE FROM system_logs WHERE source = 'CHAT_MESSENGER' AND psid = ?", [psid]);
    await runSql("DELETE FROM bot_limits WHERE psid = ?", [psid]);
  ...
  const isRegistering = !isVerified && (session?.state === 'AWAITING_ALL_IN_ONE' || session?.state === 'AWAITING_OTP');

  // 3. Daily Message Quota (10 Unverified / 15 Verified) - 8:00 AM PHT Reset
  // Unverified users with active registration in progress are completely exempt
  if (!isRegistering) {
    ...
    if (!quotaCheck.allowed) {
      if (!quotaCheck.silentDrop) {
        ...
        await sendTextMessage(
          psid,
          `You have reached today's conversation limit (${quotaCheck.limit} messages).\n\nTo ensure fair access and dependable service, your daily quota resets tomorrow at 8:00 AM PHT.\n\nThank you for your understanding!`
        );
      } else {
  ...
  // Direct Promo Code Redemption: Users can send the promo code directly without /redeem
  if (!isRegistering && text && !text.includes('\n')) {
    const rawCandidate = text.trim();
    if (rawCandidate.length >= 2 && rawCandidate.length <= 30 && !rawCandidate.includes(' ')) {
      const promoCandidate = rawCandidate.toUpperCase();
      const matchedPromoRows = await runSql("SELECT code FROM promo_codes WHERE UPPER(code) = ? LIMIT 1", [promoCandidate]);
      if (matchedPromoRows && matchedPromoRows.length > 0) {
        const foundCode = matchedPromoRows[0].code;
        if (!isVerified) {
          await sendTextMessage(
            psid,
            `Promo code "${foundCode}" recognized! Please verify your missionary account first to claim your reward points.`,
            [{ title: "Verify & Join (+1)", payload: "GET_STARTED" }]
          );
          return;
        }
        await handlePromoRedeem(psid, missionary, foundCode);
        return;
      }
    }
  }
  ...
  if (cleanPayload === 'PROMO_INFO' || lower === 'redeem promo') {
    ...
    const qr = await getVerifiedQuickReplies(psid);
    await sendTextMessage(
      psid,
      `${toUnicodeBold("Redeem Promo Code")}\n\nTo redeem a freebie promo code, reply with your code (or /redeem YOURCODE).\n\nExample:\nSPECIALGIFT`,
      qr.length > 0 ? qr : undefined
    );
    return;
  }
  ...
  if (isVerified) {
    if (cleanPayload.startsWith("CLAIM_ITEM_")) {
      const qr = await getVerifiedQuickReplies(psid);
      const activeQr = qr.length > 0 ? qr : undefined;

      // Anti-Double-Spend: Prevent concurrent redemption requests for the same PSID
      if (inFlightRedemptions.has(psid)) {
        await sendTextMessage(psid, "Your redemption request is currently being processed. Please wait a moment...", activeQr);
        return;
      }
      inFlightRedemptions.add(psid);
      ...
      try {
        ...
        if (!prod) {
          await sendTextMessage(psid, "Selected reward item is no longer available.", activeQr);
          return;
        }
        ...
        if (userPts < cost) {
          ...
          await sendTextMessage(psid, `You need ${diff} more Point(s) to claim "${prod.name}".\n\nTip: Invite a companion to get +1 Point instantly!`, activeQr);
          return;
        }

        // Atomic Point Deduction:
        // Conditional update ensures points are only decremented if balance >= cost
        await runSql("UPDATE missionaries SET points = points - ? WHERE psid = ? AND points >= ?", [cost, psid, cost]);

        // Verify that the database actually reduced the balance before creating the order
        const verifiedRecord = (await runSql("SELECT points FROM missionaries WHERE psid = ? LIMIT 1", [psid]))[0];
        const newPts = Number(verifiedRecord?.points);

        if (isNaN(newPts) || newPts !== userPts - cost) {
          await logEvent('WARN', `[REDEMPTION_ABORT] Point balance mismatch or concurrent double-spend detected for PSID ${psid}. Expected: ${userPts - cost}, Actual: ${newPts}`, psid);
          await sendTextMessage(psid, "Unable to complete redemption. Your reward points balance may have changed. Please tap 'Check' to view your current balance.", activeQr);
          return;
        }
        ...
        inFlightRedemptions.delete(psid);
      }
    }
    ...
    // Check, Dashboard, or any chat from verified missionary renders the unified hub
    const currentM = (await runSql("SELECT points, pending_ref_notices FROM missionaries WHERE psid = ? LIMIT 1", [psid]))?.[0];
    const pendingNotices = Number(currentM?.pending_ref_notices || missionary.pending_ref_notices || 0);
    const hasPendingReferral = pendingNotices > 0;

    // Daily 1-check view limit check
    const viewLimit = await checkDailyViewLimit(psid, 1);
    if (!viewLimit.allowed && !hasPendingReferral) {
      if (viewLimit.shouldWarn) {
        await logSystemEvent('WARN', `[CHECK_LIMIT_WARNED] [PSID:${psid}] Check daily limit reached (1/1) - warning notice sent`, psid);
        await sendTextMessage(
          psid,
          "You have already checked your rewards balance today (limit: 1 check per day).\n\nTo ensure dependable service for all missionaries, your daily check resets tomorrow at 8:00 AM PHT.\n\nThank you for your understanding!"
        );
      } else {
        await logSystemEvent('INFO', `[CHECK_LIMIT_SILENT] [PSID:${psid}] Check request silently dropped. 1-check per day limit used until 8:00 AM PHT.`, psid);
      }
      return;
    }
  ...
  if (!session) {
    session = (await runSql("SELECT * FROM sessions WHERE psid = ? LIMIT 1", [psid]))?.[0] || null;
  }
  if (!session) {
  ...
  // Preserve companion referral link code in session
  if (cleanRef && cleanRef !== 'JOIN' && (!session.invite_code || session.invite_code === 'JOIN')) {
    await runSql("UPDATE sessions SET invite_code = ? WHERE psid = ?", [cleanRef, psid]);
    session.invite_code = cleanRef;
  }

  // 1. Unified Registration & Terms Setup Trigger
  if (cleanPayload === 'GET_STARTED' || lower === 'get started' || lower === 'join' || lower === 'register') {
  ...
  if (session.state === 'AWAITING_DETAILS' || session.state === 'AWAITING_ALL_IN_ONE') {
    ...
    const parsed = parseMissionaryRegistrationDetails(text, session?.invite_code);

    if (!parsed) {
      await sendTextMessage(
        psid,
        "All 4 missionary details are strictly required to create your account. Please send all 4 details in one message:\n\n1. Title & Full Name (e.g. Elder John Smith or Sister Jane Doe)\n2. Email (@missionary.org)\n3. Month Batch (e.g. December 2026)\n4. Referral Code (e.g. TCRP50 or your companion's code)\n\nExample:\nElder John Smith\njohn.smith@missionary.org\nDecember 2026\nTCRP50\n\nNote: By entering this information, you agree to our Terms of Service & Privacy Policy.",
        [{ title: "Cancel", payload: "RESET_SESSION" }]
      );
      return;
    }

    const { emailInput, titleName, batchMonth, referralCode } = parsed;
  ...
  if (session.state === 'AWAITING_OTP') {
    ...
    if (cleanPayload === 'EDIT_DETAILS' || lower === 'edit') {
      ...
      await sendTextMessage(
        psid,
        "Please reply with all 4 updated missionary details in one message:\n\n1. Title & Full Name (e.g. Elder John Smith or Sister Jane Doe)\n2. Email (@missionary.org)\n3. Month Batch (e.g. December 2026)\n4. Referral Code (e.g. TCRP50 or your companion's code)",
        [{ title: "Cancel", payload: "RESET_SESSION" }]
      );
      return;
    }
    ...
    if (cleanedCode === session.otp_code) {
      ...
      const rawTitle = session.temp_title || 'Elder Missionary';
      const parsed = parseMissionaryName(rawTitle);
      const name = parsed.valid ? parsed.name : rawTitle;
      const firstName = parsed.valid ? parsed.firstName : '';
      const lastName = parsed.valid ? parsed.lastName : rawTitle.replace(/^(elder|sister)\s+/i, '').trim();
      const cohort = parsed.valid ? parsed.cohort : ((rawTitle.toLowerCase().includes('sister')) ? 'sister' : 'elder');
      ...
      const initialSendDate = getFirstDispatchDate(batch);

      const existingRows = await runSql("SELECT email FROM missionaries WHERE LOWER(email) = LOWER(?) LIMIT 1", [session.temp_email || '']);
      const isNewMissionary = !existingRows || existingRows.length === 0;

      if (isNewMissionary && session.invite_code && session.invite_code.toUpperCase() !== refCode.toUpperCase()) {
        await runSql("UPDATE missionaries SET points = points + 1, pending_ref_notices = COALESCE(pending_ref_notices, 0) + 1 WHERE UPPER(referral_code) = ? AND LOWER(email) != LOWER(?)", [session.invite_code.toUpperCase(), session.temp_email || '']);
      }

      await runSql(`
        INSERT INTO missionaries (email, name, first_name, last_name, cohort, batch_month, points, referral_code, psid, status, max_months, next_send_date)
        VALUES (?, ?, ?, ?, ?, ?, 1, ?, ?, 'active', ?, ?)
        ON CONFLICT(email) DO UPDATE SET 
          ...
          next_send_date = CASE WHEN (months_sent = 0 OR months_sent IS NULL) AND last_sent_at IS NULL THEN excluded.next_send_date ELSE missionaries.next_send_date END
      `, [session.temp_email, name, firstName, lastName, cohort, batch, refCode, psid, maxMonths, initialSendDate]);
