...
export async function handleBotMessage(psid, rawMessage = '', payload = null, referralParam = '') {
  ...
  // 1. GLOBAL COMMAND: /delete_account
  ...
  // 3. TERMS & ONBOARDING QUICK REPLIES (Processed even if unverified)
...
${defaultCode}`;
  ...
  if (cleanPayload === 'TERMS_DECLINE' || lower === 'decline') {
  ...
  // 4. VERIFIED USER ACTIONS & CAROUSELS
  if (isVerified) {
    ...
    // FAQs
    if (cleanPayload === 'FAQS_PAYLOAD' || cleanPayload === 'MENU_FAQS' || lower === 'faqs' || lower === 'faq' || lower.includes('about tcrp')) {
    ...
    // Rewards Catalog
    if (cleanPayload === 'DISCOVER_PAYLOAD' || cleanPayload === 'MENU_REWARDS' || lower === 'rewards' || lower === 'catalog' || lower.includes('discover')) {
  ...
  // 5. UNVERIFIED ONBOARDING STATE MACHINE
