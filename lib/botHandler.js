...
export async function buildCatalogCarousel(senderPoints = 0) {
  ...
  return {
    ...
    quick_replies: [
      { content_type: "text", title: "📊 My Dashboard", payload: "ACTION_DASHBOARD" },
      { content_type: "text", title: "🎟️ Redeem Promo", payload: "PROMO_INFO" },
      { content_type: "text", title: "💌 Invite (+1 Pt)", payload: "INVITE_COMPANION" }
    ]
  };
}
...
export async function sendRewardsCatalog(psid, missionary) {
  ...
  await sendTextMessage(
    psid,
    `🎁 ${toUnicodeBold("REWARDS CATALOG")}\n\n⭐ Your Live Balance: ${points} Point(s)\n\n• Custom Wooden Nametag — 2 Points\n• LDS Temple Keychain — 3 Points\n• Mission Ring / CTR Strap — 4 Points\n\n💡 Tip: Earn +1 Point for each companion who joins with your referral link, or claim promo codes with /redeem <CODE>!`,
    [
      { title: "📊 My Dashboard", payload: "ACTION_DASHBOARD" },
      { title: "🎟️ Redeem Promo", payload: "PROMO_INFO" },
      { title: "💌 Invite (+1 Pt)", payload: "INVITE_COMPANION" }
    ]
  );
}
...
(When you join using my code ${refCode}, we BOTH get +1 Reward Point instantly!) 🚀"`;
  ...
  await sendTextMessage(psid, invitePromoText, [
    { title: "📊 My Dashboard", payload: "ACTION_DASHBOARD" },
    { title: "🎁 Rewards Catalog", payload: "MENU_REWARDS" },
    { title: "🎟️ Redeem Promo", payload: "PROMO_INFO" }
  ]);
}
...
Tap '🎁 Rewards & Promos' in the menu or chat to browse products. When you have enough points, tap '🎁 Claim' to place your order!
...
export async function renderVerifiedDashboard(psid, missionary, prefixMsg = "") {
  ...
  const batchInfo = getFirstMonthInfo(missionary.batch_month || 'August 2026');
  const currentMissionMonth = calculateMissionMonth(missionary.batch_month || 'August 2026', maxMonths);
...
${infoHeader}
• ${missionary.name} (${isSister ? 'Sister' : 'Elder'})
• ${missionary.email}
• Batch: ${batchInfo.batchDisplay} (Month 0 • 1st: ${batchInfo.firstMonthDisplay})
• Progress: Month ${currentMissionMonth} of ${maxMonths}
• Referral Code: ${refCode}

${pointsHeader}
⭐ ${points} Reward Point(s)

Tap below to browse reward items or share your invite link:`;

  await sendTextMessage(psid, dashboardText, [
    { title: "🎁 Rewards Catalog", payload: "MENU_REWARDS" },
    { title: "💌 Invite (+1 Pt)", payload: "INVITE_COMPANION" },
    { title: "🎟️ Redeem Promo", payload: "PROMO_INFO" },
    { title: "📖 Help & FAQs", payload: "MENU_HELP" }
  ]);
}
...
export async function handlePromoRedeem(psid, missionary, codeText) {
  ...
  if (!code) {
    await sendTextMessage(psid, "⚠️ Please specify a promo code.\n\nExample:\n/redeem SPECIALGIFT", [
      { title: "📊 My Dashboard", payload: "ACTION_DASHBOARD" },
      { title: "🎁 Rewards Catalog", payload: "MENU_REWARDS" }
    ]);
    return;
...
export async function handleBotMessage(psid, rawMessage = '', payload = null, referralParam = '') {
  ...
  // 2. Anti-Flooding Rapid Burst Rate Limiter (Max 5 msgs / 60 seconds with Warn-Once + Silent Drop)
  const burstCheck = await checkBurstRateLimit(psid, 5, 60);
  if (burstCheck.limited) {
    if (!burstCheck.silentDrop) {
      await logEvent('WARN', `Rate limit exceeded (5 msgs/min) - initial warning sent`, psid);
      await sendTextMessage(psid, "⏳ You are sending messages too quickly. Please wait a moment before trying again.");
    } else {
      await logEvent('WARN', `Rate limit exceeded (5 msgs/min) - silent drop active`, psid);
    }
    return;
  }
  ...
  if (lower === '/delete_account' || lower === 'delete account') {
    ...
    await sendTextMessage(psid, "🗑️ Your missionary account and data have been completely deleted. You can re-register anytime by tapping below:", [
      { title: "✨ Get Started", payload: "GET_STARTED" }
    ]);
    return;
  }
  ...
  if (isVerified) {
    ...
    if (cleanPayload === 'MENU_REWARDS' || cleanPayload === 'DISCOVER_PAYLOAD' || lower === 'rewards' || lower === 'catalog') {
      await sendRewardsCatalog(psid, missionary);
      return;
    }
    ...
    // Default for verified missionary: render dashboard
  ...
  if (cleanPayload === 'ACTION_DASHBOARD' || cleanPayload === 'MENU_REWARDS' || cleanPayload === 'DISCOVER_PAYLOAD' || cleanPayload === 'PROMO_INFO' || cleanPayload === 'INVITE_COMPANION') {
  ...
  if (session.state === 'START' || lower === 'get started' || cleanPayload === 'GET_STARTED') {
    ...
    await sendTextMessage(
      psid,
      `✨ ${welcomeBold}\n\n📜 Terms & Verification Policy:\n• Program is exclusively for active full-time LDS missionaries.\n• An active @missionary.org email address is required.\n• Data is used strictly for missionary verification and reward deliveries.\n\nDo you agree to continue?`,
      [
  ...
  if (session.state === 'AWAITING_TERMS') {
    if (cleanPayload === 'TERMS_AGREE' || lower.includes('agree')) {
      ...
      await sendTextMessage(
        psid,
        "⚠️ You have declined the Terms & Conditions. You can restart anytime by tapping below:",
        [{ title: "✨ Get Started", payload: "GET_STARTED" }]
      );
      return;
    }
  ...
  if (session.state === 'AWAITING_DETAILS' || session.state === 'AWAITING_ALL_IN_ONE') {
    ...
    if (!emailMatch) {
      await sendTextMessage(
        psid,
        "⚠️ Missing or invalid @missionary.org email address. Please send your details following the required format:\n\nElder John Smith\nSeptember 2026\njohn.smith@missionary.org\nTCRP50",
        [{ title: "❌ Cancel", payload: "RESET_SESSION" }]
      );
      return;
    }
    ...
    if (!otpEligibility.allowed) {
      ...
      if (otpEligibility.reason === 'MAX_RESENDS_REACHED') {
        await sendTextMessage(
          psid,
          `⚠️ You have reached today's verification code limit (${otpEligibility.maxResends} codes per day).\n\nPlease try again tomorrow at 12:00 AM UTC+8.`,
          [{ title: "❌ Cancel", payload: "RESET_SESSION" }]
        );
        return;
      }
    ...
    try {
      await sendOTPEmail(emailInput, otp, titleName);
    } catch (err) {
    ...
    await sendTextMessage(
      ...
      [
        ...
        { title: "✏️ Edit Details", payload: "EDIT_DETAILS" },
        { title: "❌ Cancel", payload: "RESET_SESSION" }
      ]
    );
    return;
  ...
  if (session.state === 'AWAITING_OTP') {
    if (cleanPayload === 'RESEND_OTP' || lower === 'resend') {
      ...
      if (!recipientEmail) {
        await runSql("UPDATE sessions SET state = 'AWAITING_ALL_IN_ONE' WHERE psid = ?", [psid]);
        await sendTextMessage(psid, "⚠️ Email address missing. Please re-enter your missionary details:", [
          { title: "❌ Cancel", payload: "RESET_SESSION" }
        ]);
        return;
      }
      ...
      if (!otpEligibility.allowed) {
        ...
        if (otpEligibility.reason === 'MAX_RESENDS_REACHED') {
          await sendTextMessage(
            psid,
            `⚠️ You have reached today's limit for verification codes (${otpEligibility.maxResends} codes per day).\n\nPlease check your email for previously sent codes or try again tomorrow at 12:00 AM UTC+8.`,
            [
              { title: "✏️ Edit Details", payload: "EDIT_DETAILS" },
              { title: "❌ Cancel", payload: "RESET_SESSION" }
            ]
      ...
      try {
        await sendOTPEmail(recipientEmail, newOtp, session.temp_title || "Elder Missionary");
      } catch (err) {
