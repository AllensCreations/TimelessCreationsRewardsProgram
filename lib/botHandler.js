...
// Main Bot State Machine Handler
export async function handleBotMessage(psid, rawMessage, payload = null) {
  ...
  if (lower === 'faqs' || payload === 'FAQS_PAYLOAD') {
    await sendTextWithQuickReplies(psid, FAQ_TEXT, [
      { title: "🛍️ Catalog", payload: "CATALOG_PAYLOAD" },
      { title: "✨ Get Started", payload: "GET_STARTED" }
    ]);
    return;
  }

  // Trigger: Catalog / Products
  if (lower === 'catalog' || lower === 'dashboard' || payload === 'CATALOG_PAYLOAD') {
    ...
    await sendTextWithQuickReplies(psid, "Tap any item above to claim with your points, or type 'FAQs' for help.", [
  ...
  if (session.state === 'AWAITING_NAME_EMAIL') {
    ...
    emailInput = emailMatch[1].toLowerCase();
    nameInput = text.replace(emailMatch[0], '').replace(/\n/g, ' ').trim() || (emailInput.split('.')[0] || "Missionary");
  ...
  if (session.state === 'AWAITING_OTP') {
    ...
    if (cleanedCode === session.otp_code) {
      // Successful Verification: Register missionary in DB
      ...
      // If they had an invitation code, grant referral bonus
      ...
      await sendTextWithQuickReplies(
        psid,
        `🎉 Congratulations ${session.temp_title}! Your account is verified and active!\n\n🎁 You have received +1 Welcome Point.\n🏷️ Your Referral Code: ${refCode}\n(Share this code with fellow missionaries to earn +1 Point per referral!)`
      );
