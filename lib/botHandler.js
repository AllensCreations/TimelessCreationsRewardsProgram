...
async function renderDashboardAndCarousel(psid, prefixMessage = "") {
  ...
  if (!missionary) {
    ...
    await logSystemEvent('TURSO', `[SECURITY] Unverified PSID ${psid} blocked from bypassing to dashboard. Redirecting to verification.`);
    await sendTextWithQuickReplies(
      psid,
      "🔒 ACCESS DENIED: You are not verified.\n\nPlease enter your 6-character Invitation / Referral Code (e.g. A4K9M2) or Global Code (TCRP50) to verify your account:"
    );
    return;
  }
...
export async function handleBotMessage(psid, rawMessage, payload = null) {
  ...
  await logSystemEvent('TURSO', `EXECUTE SQL: INSERT INTO chat_messages (psid, sender, message) VALUES ('${psid}', 'user', '${text || payload}')`);
  try {
  ...
  if (lower === 'faqs' || payload === 'FAQS_PAYLOAD') {
    await sendTextWithQuickReplies(psid, "📖 FREQUENTLY ASKED QUESTIONS:\n1. What is TCRP? Missionary rewards.\n2. How do I verify? Use your referral code and @missionary.org email.", [
      { title: "✨ Get Started", payload: "GET_STARTED" }
    ]);
    return;
  }

  if (lower === 'discover' || lower === 'dashboard' || payload === 'DISCOVER_PAYLOAD' || payload === 'ACTION_DASHBOARD') {
  ...
  if (lower === 'get started' || payload === 'GET_STARTED' || session.state === 'START') {
    ...
    if (existingMissionary && existingMissionary.length > 0) {
      await renderDashboardAndCarousel(psid, `👋 Welcome back, ${existingMissionary[0].name}!`);
      return;
    }

    await runSql("UPDATE sessions SET state = 'AWAITING_REFERRAL' WHERE psid = ?", [psid]);
    await logSystemEvent('TURSO', `UPDATE sessions SET state = 'AWAITING_REFERRAL' WHERE psid = '${psid}'`);
    await sendTextWithQuickReplies(
      psid,
  ...
  if (session.state === 'AWAITING_REFERRAL') {
    ...
    await logSystemEvent('TURSO', `UPDATE sessions SET state = 'AWAITING_TERMS', invite_code = '${validCode}' WHERE psid = '${psid}'`);
  ...
  if (session.state === 'AWAITING_TERMS') {
    if (payload === 'TERMS_AGREE' || lower.includes('agree')) {
      ...
      await logSystemEvent('TURSO', `UPDATE sessions SET state = 'AWAITING_NAME_EMAIL' WHERE psid = '${psid}'`);
      await sendTextWithQuickReplies(
        psid,
  ...
  if (session.state === 'AWAITING_NAME_EMAIL') {
    ...
    if (!emailMatch) {
      await sendTextWithQuickReplies(psid, "⚠️ Invalid Email. You must provide an official @missionary.org email address.");
      return;
    }
    ...
    await logSystemEvent('TURSO', `UPDATE sessions SET state = 'AWAITING_OTP', temp_email = '${emailInput}' WHERE psid = '${psid}'`);
  ...
  if (session.state === 'AWAITING_OTP') {
    ...
    if (cleanedCode === session.otp_code) {
      ...
      `, [session.temp_email, session.temp_title, cohort, refCode, psid]);
      
      await logSystemEvent('TURSO', `INSERT INTO missionaries (email, name, points, psid) VALUES ('${session.temp_email}', '${session.temp_title}', 1, '${psid}')`);
      ...
      await sendTextWithQuickReplies(psid, "❌ Incorrect 6-digit OTP code. Please recheck your email.");
      return;
    }
  }

  await sendTextWithQuickReplies(psid, "👋 Hello! Please tap 'Get Started' to verify your missionary account.", [
    { title: "✨ Get Started", payload: "GET_STARTED" }
  ]);
}
