...
export function parseMissionaryRegistrationDetails(text) {
  ...
  if (lines.length >= 2) {
    ...
    if (parts.length >= 2) {
      ...
      if (isRef) {
        ...
        titleName = parts.slice(0, -1).join(' ');
      }
    }
  }
...
export async function handleBotMessage(psid, rawMessage = '', payload = null, referralParam = '') {
  ...
  if (isVerified) {
    ...
    // Check, Dashboard, or any chat from verified missionary renders the unified hub (subject to 1 check per day)
    const currentM = (await runSql("SELECT points, pending_ref_notices FROM missionaries WHERE psid = ? LIMIT 1", [psid]))?.[0];
    const pendingNotices = Number(currentM?.pending_ref_notices || missionary.pending_ref_notices || 0);
    let refNotice = "";
    if (pendingNotices > 0) {
    ...
    await sendVerifiedHub(psid, missionary, refNotice);
    return;
  }
  ...
  if (session.state === 'AWAITING_DETAILS' || session.state === 'AWAITING_ALL_IN_ONE') {
    ...
    const parsed = parseMissionaryRegistrationDetails(text);
