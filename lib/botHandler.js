...
export async function handleBotMessage(psid, rawMessage, payload = null) {
  ...
  if (!isVerified) {
    ...
    if (session.state === 'AWAITING_NAME_EMAIL') {
      ...
      // Updated prompt format without the dev log note
      await sendTextWithQuickReplies(
        psid,
    ...
    if (session.state === 'AWAITING_OTP') {
      if (payload === 'RESEND_OTP' || payload === 'REENTER_INFO' || lower.includes('resend') || lower.includes('change name') || lower.includes('reenter')) {
        if (payload === 'REENTER_INFO' || lower.includes('change name') || lower.includes('reenter')) {
          await runSql("UPDATE sessions SET state = 'AWAITING_NAME_EMAIL' WHERE psid = ?", [psid]);
