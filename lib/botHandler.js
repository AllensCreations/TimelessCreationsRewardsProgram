...
export async function handleBotMessage(psid, rawMessage = '', payload = null, referralParam = '') {
  ...
  if (session.state === 'AWAITING_OTP') {
    ...
    if (cleanedCode === session.otp_code) {
      ...
      await runSql(`
        INSERT INTO missionaries (email, name, first_name, last_name, cohort, batch_month, points, referral_code, psid, status, max_months)
        VALUES (?, ?, ?, ?, ?, ?, 1, ?, ?, 'active', ?)
      ...
      `, [session.temp_email, name, firstName, lastName, cohort, batch, refCode, psid, maxMonths]);
