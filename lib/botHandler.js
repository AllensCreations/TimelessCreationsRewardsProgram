...
// Helper to generate strict X#X#X# format (e.g. A4B9C2)
...
Staff will prepare your custom item. Please present Reference ID #${refId} in this chat for custom engraving details & dispatch!`;
  ...
  if (session.state === 'AWAITING_OTP') {
    ...
    if (cleanedCode === session.otp_code) {
      const refCode = generateXNXNXN(); // Strict X#X#X# (e.g. A4B9C2)
      ...
      // Attribute referral points safely
      if (session.invite_code) {
        if (session.invite_code === 'TCRP50') {
          await runSql("UPDATE global_referral_pool SET uses_count = uses_count + 1 WHERE code = 'TCRP50'");
        } else {
          // Bonus +1 point to referrer
