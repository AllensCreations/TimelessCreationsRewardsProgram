...
export async function handleBotMessage(psid, rawMessage, payload = null) {
  ...
  if (isVerified && (cleanPayload.startsWith("CLAIM_ITEM_") || lower.startsWith("claim"))) {
    ...
    if (product) {
      ...
      if (currentPts >= cost) {
        ...
        // Send automated receipt email with screenshot instructions & messenger link
  ...
  if (isVerified) {
    ...
    if (lower === 'discover' || lower === 'dashboard' || cleanPayload === 'DISCOVER_PAYLOAD' || cleanPayload === 'ACTION_DASHBOARD' || lower === 'get started') {
