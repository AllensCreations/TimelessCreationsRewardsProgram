...
6. 𝗗𝗶𝗱𝗻'𝘁 𝗴𝗲𝘁 𝘆𝗼𝘂𝗿 𝗢𝗧𝗣?
Answer: Check your spam folder in your @missionary.org inbox, or tap '🔄 Resend Code'.

7. 𝗛𝗼𝘄 𝗱𝗼 𝗜 𝗰𝗹𝗮𝗶𝗺 𝗺𝘆 𝗿𝗲𝘄𝗮𝗿𝗱?
Answer: Redeem from the catalog carousel to generate an official receipt, then present the Order ID to an admin for dispatch.
...
10. 𝗜𝘀 𝗺𝘆 𝗱𝗮𝘁𝗮 𝘀𝗲𝗰𝘂𝗿𝗲?
Answer: Yes. We encrypt and store minimal records solely for verification and point balance management.`;
}

export async function renderVerifiedDashboard(psid, missionary, prefixMsg = "") {
  // Check hourly rate limit in Philippine Time
  ...
  // Check passive pending referral notifications (anti-spam, only alerts when chatting)
  let referralBanner = "";
...
export async function handleBotMessage(psid, rawMessage = '', payload = null, referralParam = '') {
  ...
  // 2. UNVERIFIED USER: Streamlined Onboarding with Self-Healing Recovery
  ...
  // SELF-HEALING RECOVERY: If user types "Get Started" or sends a blank/generic message while stuck, re-prompt current state
  if (lower === 'get started' || cleanPayload === 'GET_STARTED' || lower === 'start' || text === ) {
...
${defaultCode}`;
  ...
  // STEP 3: OTP CONFIRMATION & PASSIVE REFERRAL NOTIFICATION TAGGING
  if (session.state === 'AWAITING_OTP') {
    ...
    if (cleanedCode === session.otp_code) {
      ...
      // Award referral bonus point & queue a passive notification for the referrer
