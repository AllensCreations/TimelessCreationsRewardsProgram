...
async function sendOtpEmail(email, otp) {
  // Print OTP directly into Turso system_logs so it streams to the Dev Log panel
...
export async function handleBotMessage(psid, rawMessage, payload = null) {
  ...
  // 1. If verified, grant full access
  if (isVerified) {
  ...
  // 2. If NOT verified, enforce conversation flow continuity based on current session state
  if (!isVerified) {
    // Universal informational commands
    if (lower === 'faqs' || payload === 'FAQS_PAYLOAD') {
    ...
    if (session.state === 'AWAITING_REFERRAL') {
      // If user typed random words/dashboard while awaiting referral, gently remind them instead of resetting or bypassing
      if (lower === 'dashboard' || lower === 'discover' || payload === 'DISCOVER_PAYLOAD') {
    ...
    if (session.state === 'AWAITING_OTP') {
      if (payload === 'RESEND_OTP' || lower.includes('resend')) {
        ...
        await sendTextWithQuickReplies(psid, `🔄 A new OTP has been generated. Check your Dev Log panel.`);
        return;
      }
      ...
      if (cleanedCode === session.otp_code) {
        ...
        await sendTextWithQuickReplies(psid, "❌ Incorrect 6-digit OTP code. Please check your Dev Log panel for the passcode and enter it below:");
        return;
      }
    }
  }
