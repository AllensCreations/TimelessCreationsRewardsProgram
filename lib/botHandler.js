...
${defaultCode}`;
  ...
  if (session.state === 'AWAITING_ALL_IN_ONE') {
    ...
    if (lines.length >= 2) {
      ...
      // Single line fallback extraction
    ...
    // Validate referral code (Self-referral check & existence)
  ...
  // STEP 3: OTP CONFIRMATION & ANTI-EXPLOIT LINKING
  if (session.state === 'AWAITING_OTP') {
    ...
    if (cleanedCode === session.otp_code) {
      ...
      // Award referral bonus point to referrer if not rejoining
