...
export function parseMissionaryRegistrationDetails(text, fallbackTitle = "Elder Missionary", fallbackBatch = null, fallbackRef = "TCRP50") {
  const emailMatch = text.match(/([a-zA-Z0-9._%+-]+@missionary\.org)/i);
  if (!emailMatch) return null;

  const emailInput = emailMatch[1].toLowerCase();
  const currentBatchMonth = new Date().toLocaleDateString('en-US', { month: 'long', year: 'numeric', timeZone: 'Asia/Manila' }) || "September 2026";
  let batchMonth = fallbackBatch || currentBatchMonth;
  let titleName = fallbackTitle;
  let referralCode = fallbackRef;
  ...
  if (lines.length >= 2) {
    const nonEmailLines = lines.filter(l => !l.toLowerCase().includes('@missionary.org'));
    let nameFound = false;

    for (const line of nonEmailLines) {
      const mMatch = line.match(monthRegex);
      if (mMatch) {
        const mName = mMatch[1].charAt(0).toUpperCase() + mMatch[1].slice(1).toLowerCase();
        const mYear = mMatch[2] || new Date().getFullYear().toString();
        batchMonth = `${mName} ${mYear}`;
      } else if (!nameFound) {
        titleName = line;
        nameFound = true;
      } else {
    ...
    let remaining = text.replace(emailMatch[0], '');
    const mMatch = remaining.match(monthRegex);
    if (mMatch) {
      const mName = mMatch[1].charAt(0).toUpperCase() + mMatch[1].slice(1).toLowerCase();
      const mYear = mMatch[2] || new Date().getFullYear().toString();
      batchMonth = `${mName} ${mYear}`;
    ...
    if (parts.length >= 2) {
      ...
      if (/^[A-Za-z0-9]{4,12}$/.test(last) && !/^(elder|sister)$/i.test(last)) {
...
export async function handleBotMessage(psid, rawMessage = '', payload = null, referralParam = '') {
  ...
  if (session.state === 'AWAITING_DETAILS' || session.state === 'AWAITING_ALL_IN_ONE') {
    ...
    const parsed = parseMissionaryRegistrationDetails(
      text,
      session.temp_title || "Elder Missionary",
      session.temp_batch,
      cleanRef || session.invite_code || "TCRP50"
    );
  ...
  if (session.state === 'AWAITING_OTP') {
    ...
    const parsed = parseMissionaryRegistrationDetails(
      ...
      session.temp_title || "Elder Missionary",
      session.temp_batch,
      session.invite_code || cleanRef || "TCRP50"
    );
