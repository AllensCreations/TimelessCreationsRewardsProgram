...
async function sendFbGraphMessage(psid, messagePayload) {
  ...
  if (!PAGE_ACCESS_TOKEN || PAGE_ACCESS_TOKEN.startsWith('EAA_MOCK')) {
...
export async function handleBotMessage(psid, rawMessage = '', payload = null, referralParam = '') {
  ...
  // 2. UNVERIFIED USER: Strictly route into onboarding
  await logEvent('INFO', `User unverified. Routing to Onboarding State Machine.`, psid);
  ...
  // Auto-fill referral code if passed via referral parameter
  ...
  if (session.state === 'AWAITING_NAME_EMAIL') {
    ...
    await logEvent('INFO', `🔐 [OTP GENERATED FOR TEST] Email: ${emailInput} | Code: ${otp}`, psid);
  ...
  if (session.state === 'AWAITING_OTP') {
    ...
    if (cleanedCode === session.otp_code) {
      ...
      // Credit inviter if valid code was provided
  ...
  // Fallback redirection to Onboarding
  await runSql("UPDATE sessions SET state = 'AWAITING_REFERRAL' WHERE psid = ?", [psid]);
  await sendTextMessage(
    psid,
