...
export async function buildCatalogCarousel(senderPoints = 0) {
  ...
  return {
    ...
    quick_replies: [
      ...
      { content_type: "text", title: "Redeem Promo", payload: "PROMO_INFO" },
...
export async function sendRewardsCatalog(psid, missionary) {
  ...
  await sendTextMessage(
    ...
    [
      ...
      { title: "Redeem Promo", payload: "PROMO_INFO" },
...
export async function handlePromoRedeem(psid, missionary, codeText) {
  ...
  if (Number(promo.claimed_count) >= Number(promo.max_users)) {
    await sendTextMessage(psid, `Promo code "${code}" has reached its maximum user claim limit (${promo.max_users}/${promo.max_users}).`, [
      { title: "My Dashboard", payload: "ACTION_DASHBOARD" },
      { title: "Rewards Catalog", payload: "MENU_REWARDS" }
    ]);
    return;
  ...
  if (alreadyClaimed && alreadyClaimed.length > 0) {
    await sendTextMessage(psid, `You have already redeemed promo code "${code}".`, [
      { title: "My Dashboard", payload: "ACTION_DASHBOARD" },
      { title: "Rewards Catalog", payload: "MENU_REWARDS" }
    ]);
    return;
...
export async function handleBotMessage(psid, rawMessage = '', payload = null, referralParam = '') {
  ...
  if (isVerified) {
    ...
    if (cleanPayload === 'ACTION_DASHBOARD' || cleanPayload === 'MENU_DASHBOARD' || lower === 'dashboard' || lower === 'my dashboard') {
