...
import { resolveBotMaintenanceState } from './handlers/systemHandler.js';
...
• ${missionary.name}
• ${missionary.email}
• Batch: ${batchInfo.batchDisplay} (1st Month: ${batchInfo.firstMonthDisplay})
• Mission Progress: Month ${currentMissionMonth} of ${maxMonths}
...
export async function handleBotMessage(psid, rawMessage = '', payload = null, referralParam = '') {
  ...
  // Messenger Bot Maintenance Mode Check
  const botMaintState = await resolveBotMaintenanceState();
  if (botMaintState === 'ON') {
    await logEvent('WARN', `Messenger bot in maintenance mode - notice dispatched to PSID`, psid);
    await sendTextMessage(
      psid,
  ...
  if (isVerified) {
    if (cleanPayload.startsWith("CLAIM_ITEM_")) {
      ...
      await sendTextMessage(
        psid,
        `🎉 ${claimTitle}\n\nYou have successfully claimed:\n📦 Item: ${prod.name}\n⭐ Points Used: ${cost} PTS\n🆔 Order ID: ${orderId}\n\n💌 Please check your email inbox (${missionary.email}) for your redemption receipt!`,
        [
          { title: "📊 Dashboard", payload: "ACTION_DASHBOARD" },
          { title: "🛍️ Discover", payload: "DISCOVER_PAYLOAD" }
        ]
