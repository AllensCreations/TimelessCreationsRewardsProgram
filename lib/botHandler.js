...
// In-memory OTP Cooldown Tracker (60 seconds)
const otpCooldowns = new Map();

async function callSendAPI(payload) {
  if (!PAGE_ACCESS_TOKEN) return;
  try {
    try { await runSql("INSERT INTO chat_messages (psid, sender, message) VALUES (?, 'bot', ?)", [payload.recipient?.id, payload.message?.text || "[Attachment/Carousel]"]); } catch(e){} 
...
export async function handleBotMessage(psid, rawMessage, payload = null) {
  ...
  try { await runSql("INSERT INTO chat_messages (psid, sender, message) VALUES (?, 'user', ?)", [psid, text || payload || "[Action]"]); } catch(e){}

  // 1. AUTOMATED CLAIM HANDLING (REDEEM POSTBACK)
  if (payload && (payload.startsWith('REDEEM_ITEM_') || payload.startsWith('REDEEM_'))) {
    ...
    if (!product) {
      await sendTextWithQuickReplies(psid, "⚠️ Reward item not found in catalog.", [{ title: "🛍️ Discover", payload: "DISCOVER_PAYLOAD" }]);
      return;
    }
    ...
    // Deduct points & Generate Reference ID
    const newPoints = currentPoints - cost;
    const refId = "TC-" + Math.random().toString(36).substring(2, 6).toUpperCase();
...
Staff will prepare your custom item. Please present Reference ID #${refId} in this chat for custom engraving details & dispatch!`;
  ...
  // FLOW 4: NAME AND EMAIL INPUT WITH OTP RATE LIMITING
  if (session.state === 'AWAITING_NAME_EMAIL') {
    ...
    const now = Date.now();
    const lastSent = otpCooldowns.get(psid) || 0;
    if (now - lastSent < 60000) {
      const waitSec = Math.ceil((60000 - (now - lastSent)) / 1000);
      await sendTextWithQuickReplies(psid, `⏳ Please wait ${waitSec} second(s) before requesting another verification code.`);
      return;
    }

    const otp = Math.floor(100000 + Math.random() * 900000).toString();
    otpCooldowns.set(psid, now);
  ...
  // FLOW 5: OTP VERIFICATION & RATE-LIMITED RESEND
  if (session.state === 'AWAITING_OTP') {
    ...
    if (cleanedCode === session.otp_code) {
      ...
      // Auto-purge temporary session record
