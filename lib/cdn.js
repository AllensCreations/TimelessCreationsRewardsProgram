...
export function getCdnImageUrl(urlOrPath, fallbackName = "image.jpg") {
  ...
  // Google Drive optimization: extract file ID and request compressed =s400 thumbnail
  let targetOrigin = trimmed;
  let fileKey = "asset";
  const driveMatch = trimmed.match(/\/d\/([a-zA-Z0-9_-]+)/) || trimmed.match(/[?&]id=([a-zA-Z0-9_-]+)/);
  
  if (driveMatch) {
    fileKey = `drive_${driveMatch[1]}`;
    targetOrigin = `https://lh3.googleusercontent.com/d/${driveMatch[1]}=s400`;
  } else {
    fileKey = `ext_${trimmed.replace(/[^a-zA-Z0-9_-]/g, "_").slice(-24)}`;
  }

  const ext = trimmed.includes(".png") ? ".png" : (trimmed.includes(".webp") ? ".webp" : ".jpg");
  return `${R2_WORKER_BASE}/drips/${fileKey}${ext}?origin=${encodeURIComponent(targetOrigin)}`;
}
