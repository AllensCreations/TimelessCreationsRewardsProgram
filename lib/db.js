...
function executeInMemoryFallback(query, params = []) {
  ...
  if (qLower.includes('into system_logs')) {
    memStore.system_logs.push({
      id: memStore.system_logs.length + 1,
      level: params[0] || 'INFO',
      message: params[1] || '',
  ...
  // email_events queries
  if (qLower.includes('into email_events')) {
    const id = (memStore.email_events || []).length + 1;
    const evt = {
      id,
      email: params[0] || '',
    ...
    if (!memStore.email_events) memStore.email_events = [];
    memStore.email_events.unshift(evt);
    return [{ ok: true, id }];
  }
  if (qLower.includes('from email_events')) {
    if (qLower.includes('count(*)')) {
      ...
      const list = (memStore.email_events || []).filter(e => {
    ...
    const events = (memStore.email_events || []).slice(0, 15).map(e => {
  ...
  if (qLower.includes('update bot_daily_views set warned = 1') || qLower.includes('update bot_daily_views set warned =')) {
    const sender_id = String(params[0] || '');
    const view_date = String(params[1] || '');
    const key = `${sender_id}:${view_date}`;
    ...
    if (r) {
      r.warned = 1;
      memStore.bot_daily_views.set(key, r);
    }
    return [{ ok: true }];
  }
  ...
  _schemaInitPromise = (async () => {
    try {
      const statements = [
        ...
        { sql: "CREATE TABLE IF NOT EXISTS chat_messages (id integer PRIMARY KEY AUTOINCREMENT, psid text, sender text CHECK(sender IN ('user','bot')), message text, created_at text DEFAULT CURRENT_TIMESTAMP);" },
        { sql: "CREATE TABLE IF NOT EXISTS bot_rate_limits (psid text PRIMARY KEY, msg_count integer DEFAULT 0, window_start integer DEFAULT 0, warned integer DEFAULT 0, identifier text, action text, count integer DEFAULT 0);" },
        { sql: "CREATE TABLE IF NOT EXISTS bot_daily_user_quotas (psid text, quota_date text, msg_count integer DEFAULT 0, warned integer DEFAULT 0, otp_resend_count integer DEFAULT 0, PRIMARY KEY(psid, quota_date));" },
        { sql: "CREATE TABLE IF NOT EXISTS bot_daily_views (sender_id text, view_date text, view_count integer DEFAULT 0, warned integer DEFAULT 0, PRIMARY KEY(sender_id, view_date));" },
        ...
        { sql: "CREATE TABLE IF NOT EXISTS promo_redemptions (code text, psid text, created_at text DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY(code, psid));" },
        ...
        { sql: "CREATE TABLE IF NOT EXISTS email_events (id integer PRIMARY KEY AUTOINCREMENT, email text, event_type text, subject text, message_id text, ip text, created_at text DEFAULT CURRENT_TIMESTAMP);" },
