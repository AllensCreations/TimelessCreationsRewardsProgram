...
(async function initDatabaseSchema() {
  try {
    // 1. Clean up unused bot tables
    await runSql("DROP TABLE IF EXISTS chat_messages;").catch(() => {});
    await runSql("DROP TABLE IF EXISTS sessions;").catch(() => {});
    await runSql("DROP TABLE IF EXISTS bot_hourly_views;").catch(() => {});
    await runSql("DROP TABLE IF EXISTS bot_rate_limits;").catch(() => {});
    ...
    // 3. Ensure column migrations on drip_messages & missionaries
