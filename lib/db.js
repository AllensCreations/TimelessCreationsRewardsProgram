...
// Auto-load .env if it exists locally in development
if (fs.existsSync('.env') && !process.env.TURSO_DATABASE_URL) {
  try {
...
const getHttpUrl = (url) => {
  if (!url) return '';
  let clean = url.replace(/^libsql:\/\//, 'https://').replace(/^turso:\/\//, 'https://');
  if (clean.endsWith('/')) clean = clean.slice(0, -1);
  return clean;
};

export async function queryTurso(statements) {
  const baseUrl = getHttpUrl(TURSO_URL);
  if (!baseUrl) {
    throw new Error("TURSO_DATABASE_URL environment variable is missing or empty.");
  }
...
async function ensureWalMode() {
  try { await runSql("PRAGMA journal_mode=WAL;"); } catch(e){}
}
ensureWalMode();
