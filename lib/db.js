...
function executeInMemoryFallback(query, params = []) {
  ...
  if (qLower.includes('into missionaries')) {
    ...
    if (emailParam) {
      ...
      const psidParam = (params || []).find(p => typeof p === 'string' && (p.startsWith('TEST_') || /^\d{10,}$/.test(p)));
      const refParam = (params || []).find(p => typeof p === 'string' && p.length >= 4 && p.length <= 12 && p === p.toUpperCase() && !p.includes('@') && !p.startsWith('TEST_') && !['ELDER', 'SISTER', 'ACTIVE'].includes(p));
