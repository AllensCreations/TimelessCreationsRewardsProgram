...
function executeInMemoryFallback(query, params = []) {
  ...
  if (qLower.includes('into sessions')) {
    ...
    memStore.sessions.set(psid, { psid, state, last_otp_at: lastOtpAt });
    return [{ ok: true }];
  }
  if (qLower.includes('update sessions')) {
    const psid = params[params.length - 1];
    let session = memStore.sessions.get(psid) || { psid, state: 'START' };
    if (qLower.includes('state = ?')) {
      session.state = params[0];
    } else if (qLower.includes("state = '")) {
      const match = q.match(/state\s*=\s*'([^']+)'/i);
      if (match) session.state = match[1];
    }
    if (qLower.includes('temp_email = ?') && params.length >= 6) {
      session.temp_title = params[0];
      session.temp_email = params[1];
      session.temp_batch = params[2];
      session.invite_code = params[3];
      session.otp_code = params[4];
    } else if (qLower.includes('otp_code = ?')) {
      session.otp_code = params[0];
    }
  ...
  if (qLower.includes('into bot_rate_limits')) {
    ...
    memStore.bot_rate_limits.set(psid, { psid, msg_count: 1, window_start });
    return [{ ok: true }];
  }
  if (qLower.includes('update bot_rate_limits')) {
    if (qLower.includes('msg_count = 1')) {
      ...
      const psid = params[0];
      const r = memStore.bot_rate_limits.get(psid) || { psid, msg_count: 0, window_start: Math.floor(Date.now() / 1000) };
