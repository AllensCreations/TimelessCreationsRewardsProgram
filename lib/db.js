...
function executeInMemoryFallback(query, params = []) {
  ...
  if (qLower.includes('into missionaries')) {
    ...
    if (emailParam) {
      const existing = memStore.missionaries.find(m => (m.email || '').toLowerCase() === emailParam.toLowerCase());
      if (!existing) {
        memStore.missionaries.push({
          email: emailParam,
          name: params[1] || 'Missionary',
          cohort: 'elder',
          points: 0,
          alive: 1
        });
  ...
  if (qLower.includes('update missionaries')) {
    const emailParam = (params || []).find(p => typeof p === 'string' && p.includes('@'));
    const targetEmail = emailParam ? emailParam.toLowerCase().trim() : '';
    const m = memStore.missionaries.find(x => (x.email || '').toLowerCase().trim() === targetEmail);

    if (qLower.includes('points = max(0, points + ?)')) {
      const delta = Number(params[0]) || 0;
      if (m) {
        m.points = Math.max(0, (m.points || 0) + delta);
      }
    }
    if (qLower.includes('open_count')) {
      if (m) {
        m.open_count = (Number(m.open_count) || 0) + 1;
        m.last_opened_at = new Date().toISOString();
      }
    }
    if (qLower.includes('months_sent')) {
      if (m) {
        m.months_sent = Number(params[0]) || (Number(m.months_sent) || 0) + 1;
        m.last_sent_at = new Date().toISOString();
      }
    }
    if (qLower.includes('status =')) {
      const match = q.match(/status\s*=\s*'([^']+)'/i);
      if (match && m) {
        m.status = match[1];
      }
    }
  ...
  // email_opens
  if (qLower.includes('into email_opens')) {
    memStore.email_opens = memStore.email_opens || [];
    memStore.email_opens.push({
      email: params[0] || '',
      month: params[1] || 0,
      template_type: params[2] || 'drip',
      source: params[3] || 'pixel',
      user_agent: params[4] || '',
      ip_hash: params[5] || '',
  ...
  if (qLower.includes('from email_opens')) {
    memStore.email_opens = memStore.email_opens || [];
    if (qLower.includes('where email =') || qLower.includes('where lower(email) =')) {
      const targetEmail = (params[0] || '').toLowerCase().trim();
      return memStore.email_opens.filter(x => (x.email || '').toLowerCase().trim() === targetEmail);
    }
    return memStore.email_opens;
  }
...
(async function initDatabaseSchema() {
  try {
    ...
    await runSql("ALTER TABLE missionaries ADD COLUMN last_opened_at text;").catch(() => {});
    await runSql("ALTER TABLE missionaries ADD COLUMN open_count integer DEFAULT 0;").catch(() => {});
    ...
    await runSql(`
      CREATE TABLE IF NOT EXISTS email_opens (
        ...
        template_type text DEFAULT 'drip',
    ...
    `).catch(() => {});
    await runSql("CREATE INDEX IF NOT EXISTS idx_email_opens ON email_opens (email, month);").catch(() => {});
    ...
    // Ensure September cohort missionaries not yet dispatched are scheduled for their 1st month (October)
    await runSql(`
      UPDATE missionaries 
      SET next_send_date = '2026-10-09'
      WHERE (months_sent = 0 OR months_sent IS NULL)
        AND (last_sent_at IS NULL OR last_sent_at = '')
        AND LOWER(batch_month) LIKE '%september%'
        AND (next_send_date IS NULL OR next_send_date < '2026-10-01')
    `).catch(() => {});
  } catch (_) {}
})();
