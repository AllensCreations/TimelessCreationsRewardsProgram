...
export async function handleCatalogAction(action, req, bodyData) {
  ...
  if (action === "update_order") {
    const { order_id, name, email, items_json, status } = bodyData;
    const cleanStatus = (status || 'PENDING').toUpperCase();
    const itemsList = Array.isArray(items_json) ? items_json.map(i => `${i.qty}x ${i.name}`).join(', ') : '';
    
    await runSql(`
      UPDATE orders 
      SET name = COALESCE(?, name), email = COALESCE(?, email), items_json = ?, item = ?, status = ?
      WHERE order_id = ?
    `, [name, email, JSON.stringify(items_json || []), itemsList, cleanStatus, order_id]);
