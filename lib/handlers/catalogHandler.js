...
export async function persistDripRewardsVisibility(visible) {
  ...
  try {
    ...
    `, ['drip_rewards_visible', val]).catch(async () => {
...
export async function handleCatalogAction(action, req, bodyData) {
  ...
  if (action === "sync_catalog" || action === "save_products") {
    ...
    if (bodyData.drip_rewards_visible !== undefined || req.body?.drip_rewards_visible !== undefined) {
      ...
      const months = bodyData.months || req.body?.months;
      if (Array.isArray(months) && months.length > 0) {
        for (const m of months) {
          const metaKey = `drip_${m}_highlight_meta`;
          const row = (await runSql("SELECT value FROM system_settings WHERE key = ?", [metaKey]).catch(() => []))[0];
          let currentMeta = {};
          try { if (row && row.value) currentMeta = JSON.parse(row.value); } catch(_) {}
          currentMeta.drip_rewards_visible = isVisible;
          const metaVal = JSON.stringify(currentMeta);
          await runSql(`
            INSERT INTO system_settings (key, value) VALUES (?, ?)
            ON CONFLICT(key) DO UPDATE SET value = excluded.value
  ...
  if (action === "get_orders") {
    const orders = await cache.getOrSet("orders_list", async () => {
      return (await runSql("SELECT * FROM orders WHERE type = 'REWARD' OR type IS NULL ORDER BY CASE WHEN UPPER(status) = 'PENDING' THEN 0 ELSE 1 END ASC, created_at ASC")) || [];
    }, 20000, ["orders"]);
  ...
  if (action === "update_order_status") {
    ...
    if (existing) {
      const oldStatus = (existing.status || 'PENDING').toUpperCase();
      const pointsCost = Number(existing.points_cost) || 0;
      if (cleanStatus === 'CANCELLED' && oldStatus !== 'CANCELLED' && oldStatus !== 'DELIVERED') {
        if (pointsCost > 0 && existing.email) {
          await runSql("UPDATE missionaries SET points = points + ? WHERE LOWER(email) = LOWER(?)", [pointsCost, existing.email]);
          cache.invalidateTag("missionaries");
        }
      } else if (oldStatus === 'CANCELLED' && cleanStatus !== 'CANCELLED') {
        if (pointsCost > 0 && existing.email) {
          await runSql("UPDATE missionaries SET points = MAX(0, points - ?) WHERE LOWER(email) = LOWER(?)", [pointsCost, existing.email]);
          cache.invalidateTag("missionaries");
        }
      }
    }
  ...
  if (action === "update_order") {
    ...
    if (existing) {
      const oldStatus = (existing.status || 'PENDING').toUpperCase();
      const points = Number(points_cost !== undefined ? points_cost : existing.points_cost) || 0;
      const targetEmail = email || existing.email;
      if (cleanStatus === 'CANCELLED' && oldStatus !== 'CANCELLED' && oldStatus !== 'DELIVERED') {
        if (points > 0 && targetEmail) {
          await runSql("UPDATE missionaries SET points = points + ? WHERE LOWER(email) = LOWER(?)", [points, targetEmail]);
          cache.invalidateTag("missionaries");
        }
      } else if (oldStatus === 'CANCELLED' && cleanStatus !== 'CANCELLED') {
        if (points > 0 && targetEmail) {
          await runSql("UPDATE missionaries SET points = MAX(0, points - ?) WHERE LOWER(email) = LOWER(?)", [points, targetEmail]);
          cache.invalidateTag("missionaries");
        }
      }
    }
  ...
  if (action === "delete_order") {
    const order_id = bodyData.order_id || req.query?.order_id;
    if (!order_id) return { status: 400, json: { ok: false, error: "Missing order_id" } };
    const existing = (await runSql("SELECT * FROM orders WHERE order_id = ?", [order_id]))[0];
    if (existing && existing.status === 'DELIVERED' && isDeliveredLocked(existing.delivered_at, existing.created_at)) {
    ...
    if (existing && existing.status !== 'CANCELLED' && existing.status !== 'DELIVERED') {
      const refundPoints = Number(existing.points_cost) || 0;
      if (refundPoints > 0 && existing.email) {
        await runSql("UPDATE missionaries SET points = points + ? WHERE LOWER(email) = LOWER(?)", [refundPoints, existing.email]);
        cache.invalidateTag("missionaries");
      }
    }
