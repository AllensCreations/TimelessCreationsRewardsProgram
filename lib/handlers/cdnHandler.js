...
export async function handleCdnAction(action, req, bodyData) {
  ...
  if (action === "get_cdn_config") {
    ...
    const owner = dbConfig.cdn_github_owner || process.env.CDN_GITHUB_OWNER || 'AllensCreations';
    const repo = dbConfig.cdn_github_repo || process.env.CDN_GITHUB_REPO || 'Gallery';
    const branch = dbConfig.cdn_github_branch || process.env.CDN_GITHUB_BRANCH || 'main';
    const uploadPath = dbConfig.cdn_upload_path || process.env.CDN_UPLOAD_PATH || 'assets/rewards';
    const token = dbConfig.cdn_github_token || process.env.CDN_GITHUB_TOKEN || process.env.GITHUB_TOKEN || process.env.GH_TOKEN || process.env.GITHUB_PAT || '';
    ...
    return {
      ...
      json: {
        ...
        config: {
          ...
          has_token: !!(token && token.trim().length > 5),
  ...
  if (action === "upload" || action === "cdn_upload") {
    ...
    let token = bodyData.token || process.env.CDN_GITHUB_TOKEN || process.env.GITHUB_TOKEN || process.env.GH_TOKEN || process.env.GITHUB_PAT || '';
