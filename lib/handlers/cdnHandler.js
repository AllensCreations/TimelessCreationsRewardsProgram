...
function resolveCdnConfig(dbConfig = {}) {
  const r2UrlRaw = dbConfig.cdn_r2_worker_url !== undefined
    ? dbConfig.cdn_r2_worker_url
    : (process.env.R2_WORKER_URL !== undefined ? process.env.R2_WORKER_URL : 'https://tcrp.2ndsalviejomark2019.workers.dev');
  return {
...
export async function handleCdnAction(action, req, bodyData) {
  ...
  if (action === "upload" || action === "cdn_upload") {
    ...
    const publicUrl = (r2WorkerUrl && r2WorkerUrl.startsWith('http'))
    ...
    return {
      ...
      json: {
        ...
        item: { filename: cleanFilename, direct_url: publicUrl, size_label: targetSize }
      }
    };
  }
