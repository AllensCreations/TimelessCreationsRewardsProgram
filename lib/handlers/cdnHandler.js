...
function resolveCdnConfig(dbConfig = {}) {
  const r2UrlRaw = dbConfig.cdn_r2_worker_url || process.env.R2_WORKER_URL || '';
  return {
...
export async function handleCdnAction(action, req, bodyData) {
  ...
  if (action === "get_cdn_config") {
    ...
    const { owner, repo, branch, uploadPath, r2WorkerUrl } = resolveCdnConfig(dbConfig);
  ...
  if (action === "add_direct_image" || action === "cdn_add_url") {
    ...
    const { r2WorkerUrl } = resolveCdnConfig(dbConfig);

    let finalUrl = directUrl;
    if (r2WorkerUrl && r2WorkerUrl.startsWith('http') && directUrl.includes('cdn.jsdelivr.net') && !directUrl.includes(r2WorkerUrl)) {
      const parts = directUrl.split('@');
      const subPath = parts.length > 1 ? parts[1].split('/').slice(1).join('/') : `cdn/${filename}`;
      finalUrl = `${r2WorkerUrl.replace(/\/+$/, '')}/${subPath}?origin=${encodeURIComponent(directUrl)}`;
    }

    await runSql(`
      INSERT INTO cdn_gallery (filename, direct_url, size_label, original_kb, compressed_kb)
      VALUES (?, ?, ?, 0, 0)
    `, [filename, finalUrl, label]);

    return { status: 200, json: { ok: true, direct_url: finalUrl, item: { filename, direct_url: finalUrl, size_label: label } } };
  }
  ...
  if (action === "upload" || action === "cdn_upload") {
    ...
    if (direct_url && !base64Data) {
      ...
      let finalUrl = direct_url;
      if (r2WorkerUrl && r2WorkerUrl.startsWith('http') && direct_url.includes('cdn.jsdelivr.net') && !direct_url.includes(r2WorkerUrl)) {
        const parts = direct_url.split('@');
        const subPath = parts.length > 1 ? parts[1].split('/').slice(1).join('/') : `cdn/${cleanFilename}`;
        finalUrl = `${r2WorkerUrl.replace(/\/+$/, '')}/${subPath}?origin=${encodeURIComponent(direct_url)}`;
      }

      await runSql(`
        INSERT INTO cdn_gallery (filename, direct_url, size_label, original_kb, compressed_kb)
        VALUES (?, ?, ?, ?, ?)
      `, [cleanFilename, finalUrl, targetSize || 'direct_url', Number(originalKb) || 0, Number(compressedKb) || 0]);
      return { status: 200, json: { ok: true, direct_url: finalUrl, item: { filename: cleanFilename, direct_url: finalUrl, size_label: targetSize } } };
    }
    ...
    let localRelativeUrl = `/assets/rewards/${cleanFilename}`;

    // Always persist uploaded image to local disk so it is instantly available and never 404s
    if (base64Data) {
      try {
        ...
        const pubDir = path.resolve('public/assets/rewards');
        if (!fs.existsSync(pubDir)) fs.mkdirSync(pubDir, { recursive: true });
        fs.writeFileSync(path.join(pubDir, cleanFilename), imgBuffer);

        const viewsDir = path.resolve('views/assets/rewards');
        if (!fs.existsSync(viewsDir)) fs.mkdirSync(viewsDir, { recursive: true });
        fs.writeFileSync(path.join(viewsDir, cleanFilename), imgBuffer);

        localRelativeUrl = `/assets/rewards/${cleanFilename}`;
      } catch (saveErr) {
        console.warn('[CDN] Failed to write image locally:', saveErr.message);
      }
    }

    let publicUrl = localRelativeUrl;
    let githubSynced = false;

    // If GitHub token is present, also sync to GitHub CDN repository
    if (base64Data && token) {
      ...
      try {
        ...
        if (commitRes.ok) {
          githubSynced = true;
          publicUrl = (r2WorkerUrl && r2WorkerUrl.startsWith('http'))
            ? `${r2WorkerUrl.replace(/\/+$/, '')}/${filePath}?origin=${encodeURIComponent(directUrl)}`
            : directUrl;
        } else {
          console.warn(`[CDN] GitHub commit returned ${commitRes.status}, falling back to local file storage.`);
        }
      } catch (commitErr) {
        console.warn('[CDN] GitHub commit network error, falling back to local file storage:', commitErr.message);
      }
    }
    ...
    await runSql(`
      INSERT INTO cdn_gallery (filename, direct_url, size_label, original_kb, compressed_kb)
      VALUES (?, ?, ?, ?, ?)
    `, [cleanFilename, publicUrl, targetSize || 'square_600', Number(originalKb) || 0, Number(compressedKb) || 0]);
    ...
    return {
      ...
      json: {
        ...
        local_url: localRelativeUrl,
        raw_jsdelivr_url: directUrl,
        github_synced: githubSynced,
        r2_accelerated: !!(r2WorkerUrl && r2WorkerUrl.startsWith('http')),
        item: { filename: cleanFilename, direct_url: publicUrl, local_url: localRelativeUrl, size_label: targetSize }
      }
    };
  }
