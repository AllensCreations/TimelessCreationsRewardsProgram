...
export async function handleCdnAction(action, req, bodyData) {
  if (action === "get_cdn_config") {
    const rows = await runSql("SELECT key, value FROM system_settings WHERE key LIKE 'cdn_%'");
    const config = {};
  ...
  if (action === "upload" || action === "cdn_upload") {
    ...
    if (base64Data) {
      if (!token) {
        return { status: 400, json: { ok: false, error: "GitHub Personal Access Token (CDN_GITHUB_TOKEN) is required to commit image assets to CDN repository" } };
      }
