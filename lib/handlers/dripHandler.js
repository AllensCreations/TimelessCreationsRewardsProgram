...
export async function handleDripAction(action, req, bodyData) {
  ...
  if (action === "get_drips") {
    ...
    const mergedDrips = (drips || []).map(d => {
      ...
      return {
        ...
        drip_rewards_visible: meta.drip_rewards_visible !== undefined ? meta.drip_rewards_visible : undefined,
  ...
  if (action === "apply_featured_and_rewards_to_months" || action === "apply_featured_products_to_months") {
    const {
      months = [],
      p1Name, p1Img, p1Sold,
      p2Name, p2Img, p2Sold,
    ...
    for (const m of targetMonths) {
      ...
      const metaKey = `drip_${m}_highlight_meta`;
      const existingRow = (await runSql("SELECT value FROM system_settings WHERE key = ?", [metaKey]).catch(() => []))[0];
      let metaObj = {};
      try { if (existingRow && existingRow.value) metaObj = JSON.parse(existingRow.value); } catch(_) {}

      metaObj.sold_1 = formatK(p1Sold);
      metaObj.label_2 = isEnabled ? (p2Name || '') : '';
      metaObj.img_2 = p2Img || '';
      metaObj.sold_2 = formatK(p2Sold);
      metaObj.enabled = isEnabled;
      if (drip_rewards_visible !== undefined) {
        metaObj.drip_rewards_visible = Boolean(drip_rewards_visible);
      }

      const metaVal = JSON.stringify(metaObj);
    ...
    if (drip_rewards_visible !== undefined) {
      ...
      await runSql(`
        ...
        VALUES (?, ?)
        ON CONFLICT(key) DO UPDATE SET value = excluded.value
      `, ['drip_rewards_visible', visVal]).catch(async () => {
    ...
    return {
      ...
      json: {
        ...
        message: `Applied featured products to ${targetMonths.length} month(s)!`
      }
    };
  }
