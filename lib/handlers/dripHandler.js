...
function formatK(num) {
  if (num === null || num === undefined || num === '') return "1.5k+ Sold";
    ...
    try {
      ...
      (catalog || []).forEach(p => {
        ...
        for (const [sName, cnt] of Object.entries(salesCountMap)) {
          if (sName.toLowerCase().includes(p.name.toLowerCase()) || p.name.toLowerCase().includes(sName.toLowerCase())) {
            matchedCount += cnt;
          }
        }
        ranked.push({
      ...
      for (const [sName, cnt] of Object.entries(salesCountMap)) {
        if (![...usedNames].some(un => un.includes(sName.toLowerCase()) || sName.toLowerCase().includes(un))) {
          ranked.push({
            name: sName,
            image_url: "https://lh3.googleusercontent.com/u/0/d/1F7Yb0OzuCmPO2LyZ0cMoaTM4d4rs5RFE",
            count: cnt
          });
        }
      }
      ...
      const top1 = ranked[0] || { name: "Wooden Nametag", image_url: "https://lh3.googleusercontent.com/u/0/d/1F7Yb0OzuCmPO2LyZ0cMoaTM4d4rs5RFE", count: 0 };
      const top2 = ranked[1] || { name: "POS Kit", image_url: "https://lh3.googleusercontent.com/u/0/d/101jY71PjxCwiuNznTgn7Xyc0HoXwB3WQ", count: 0 };

      const formatCount = (c) => {
        if (!c || c === 0) return "1.5k+ Sold";
        if (c >= 1000) return `${(c / 1000).toFixed(1).replace('.0', '')}k+ Sold`;
        return `${c}+ Sold`;
      };
