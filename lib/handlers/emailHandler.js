...
export async function handleEmailAction(action, req, bodyData) {
  if (action === "brevo_webhook") {
    const rawEvents = Array.isArray(bodyData) ? bodyData : (bodyData?.events || [bodyData]);
    let recorded = 0;

    for (const evt of rawEvents) {
      if (!evt || typeof evt !== 'object') continue;
      const email = (evt.email || evt.recipient || '').toLowerCase().trim();
      const eventType = (evt.event || evt.type || 'delivered').toLowerCase().trim();
      const subject = (evt.subject || evt.tag || '').trim();
      const messageId = (evt['message-id'] || evt.message_id || evt.id || '').trim();
      const ip = (evt.ip || '').trim();

      if (email) {
        await runSql(`
          INSERT INTO system_logs (source, level, email, event_type, subject, message_id, ip, message)
          VALUES ('EMAIL_BREVO', 'BREVO', ?, ?, ?, ?, ?, ?)
        `, [email, eventType, subject, messageId, ip, `[BREVO_${eventType.toUpperCase()}] ${email} - ${subject || 'Email Notification'}`]).catch(() => {});
    ...
    return { status: 200, json: { ok: true, recorded, processed: recorded } };
  }
    ...
    try {
      const payload = await cache.getOrSet("pending_emails_summary", async () => {
        ...
        const [missionaries, drips, powerState] = await Promise.all([
          runSql("SELECT email, name, last_name, first_name, cohort, batch_month, months_sent, max_months, psid, points, referral_code, is_active, status, last_sent_at, next_send_date FROM missionaries ORDER BY is_prelisted DESC, name ASC").catch(() => []),
        ...
        const processedMissionaries = (missionaries || []).map(m => {
          ...
          if (lastSentAt) {
            const rawDate = lastSentAt.slice(0, 10);
            const rawMonth = lastSentAt.slice(0, 7);
            if (rawDate === todayPhtIso) {
              sentToday = true;
            } else {
              try {
                const d = new Date(lastSentAt.replace(' ', 'T') + (lastSentAt.endsWith('Z') ? '' : 'Z'));
                const pht = new Date(d.getTime() + 8 * 3600 * 1000);
                sentToday = pht.toISOString().slice(0, 10) === todayPhtIso;
              } catch (_) {}
            }
            if (rawMonth === todayPhtIso.slice(0, 7)) {
              sentThisMonth = true;
            } else {
              try {
                const d = new Date(lastSentAt.replace(' ', 'T') + (lastSentAt.endsWith('Z') ? '' : 'Z'));
                const pht = new Date(d.getTime() + 8 * 3600 * 1000);
                sentThisMonth = pht.toISOString().slice(0, 7) === todayPhtIso.slice(0, 7);
              } catch (_) {}
            }
          }
          ...
          if (!isCompleted) {
            const firstDispatchMonth = getFirstDispatchDate(m.batch_month || 'August 2026', phtNow);
            const firstMonthPrefix = `${batchInfo.firstMonthYear}-${String(batchInfo.firstMonthNum).padStart(2, '0')}`;
            const currentCalPrefix = todayPhtIso.slice(0, 7);
            ...
            if (monthsSent === 0) {
              if (currentMissionMonth <= 0) {
                ...
                if (m.next_send_date && m.next_send_date.slice(0, 7) === firstMonthPrefix) {
                  estimatedNextDate = m.next_send_date.slice(0, 7);
                } else {
                  estimatedNextDate = firstMonthPrefix;
                ...
                if (m.next_send_date && m.next_send_date.slice(0, 7) <= currentCalPrefix) {
                  estimatedNextDate = m.next_send_date.slice(0, 7);
                } else {
                  estimatedNextDate = currentCalPrefix;
                  runSql("UPDATE missionaries SET next_send_date = ? WHERE LOWER(email) = ? AND (months_sent = 0 OR months_sent IS NULL)", [estimatedNextDate, m.email]).catch(() => {});
                }
              }
            } else {
              ...
              if (monthsSent < currentMissionMonth && !sentThisMonth) {
                // Missed past months and NOT yet dispatched this calendar month: due in current month
                estimatedNextDate = currentCalPrefix;
              } else if (lastSentAt) {
                try {
                  ...
                  if (!m.next_send_date || m.next_send_date.slice(0, 7) > expectedNext) {
                  ...
                  estimatedNextDate = m.next_send_date ? m.next_send_date.slice(0, 7) : currentCalPrefix;
                }
              } else {
                // Prelisted missionary with monthsSent > 0 but lastSentAt is null
                if (upcomingDrip) {
                  const nextDripDate = `${upcomingDrip.year}-${String(upcomingDrip.monthNum).padStart(2, '0')}`;
                  estimatedNextDate = m.next_send_date ? m.next_send_date.slice(0, 7) : nextDripDate;
                } else {
                  estimatedNextDate = m.next_send_date ? m.next_send_date.slice(0, 7) : currentCalPrefix;
                }
              }
          ...
          if (!isCompleted && upcomingDrip) {
            nextDripInfo = {
              month_num: upcomingDrip.monthNum,
              year: upcomingDrip.year,
              month_year: upcomingDrip.display,
              tenure_month: upcomingDrip.tenureMonth,
              theme: (dripMap[upcomingDrip.monthNum] || {}).theme || 'Encouragement & Points',
          ...
          if (!isActive) {
            ...
            sentTodayCount++;
          } else if (sentThisMonth) {
            scheduleStatus = 'SENT_THIS_MONTH';
            sentThisMonthCount++;
          } else if (isEligibleForDispatch && (!m.next_send_date || m.next_send_date.slice(0, 7) <= todayPhtIso.slice(0, 7))) {
            scheduleStatus = 'DUE_TODAY';
            dueTodayCount++;
            if (isSister) sistersDueToday++; else eldersDueToday++;
          } else if (daysUntilNext >= 1 && daysUntilNext <= 7) {
        ...
        return {
          ...
          stats: {
            ...
            sent_this_month: sentThisMonthCount,
            remaining_this_month: dueTodayCount,
    ...
    try {
      ...
      if (result?.ok) {
        ...
        await runSql(`
          UPDATE missionaries 
          SET months_sent = ?,
              last_sent_at = datetime('now', '+8 hours'),
        ...
        await runSql(`
          ...
          VALUES ('DISPATCH', ?, datetime('now', '+8 hours'))
  ...
  if (action === "reschedule_missionary_email") {
    try {
      ...
      if (!email) return { status: 400, json: { ok: false, error: "Missing missionary email" } };
      const monthMatch = nextSendDate.match(/^(\d{4}-\d{2})/);
      if (!monthMatch) {
        return { status: 400, json: { ok: false, error: "Invalid next_send_date format. Expected YYYY-MM (or YYYY-MM-DD)." } };
      }
      const normalizedMonth = monthMatch[1];

      await runSql("UPDATE missionaries SET next_send_date = ? WHERE LOWER(email) = ?", [normalizedMonth, email]);
      await runSql("INSERT INTO system_logs (level, message) VALUES ('INFO', ?)", [`Rescheduled next email for ${email} to ${normalizedMonth}`]);
      ...
      return {
        ...
        json: {
          ok: true,
          email,
          next_send_date: normalizedMonth,
          message: `Successfully rescheduled next email for ${email} to ${normalizedMonth}`
        }
      };
    } catch (err) {
          ...
          try {
            ...
            const recentSent = await runSql(`
              ...
              WHERE LOWER(email) = ? 
                ...
                AND (substr(last_sent_at, 1, 10) = ? OR date(datetime(last_sent_at, '+8 hours')) = ?)
            `, [emailKey, todayPhtIso, todayPhtIso]).catch(() => []);
