...
export async function handleEmailAction(action, req, bodyData) {
  if (action === "track_open" || action === "email_open" || action === "open_pixel") {
    try {
      let email = '';
      const rawE = (req.query?.e || req.query?.email || bodyData?.e || bodyData?.email || '').trim();
      if (rawE) {
        if (rawE.includes('@')) {
          email = rawE;
        } else {
          try {
            const decodedUrl = Buffer.from(rawE, 'base64url').toString('utf8');
          ...
          if (!email) {
            try {
              const decodedStd = Buffer.from(rawE, 'base64').toString('utf8');
              if (decodedStd.includes('@')) {
                email = decodedStd;
              }
            } catch (_) {}
          }
      ...
      const month = Number(req.query?.m || req.query?.month || bodyData?.month) || 0;
      const templateType = (req.query?.t || req.query?.template_type || bodyData?.template_type || 'drip').toLowerCase().trim();

      if (email && email.includes('@')) {
        await runSql(
          "UPDATE missionaries SET open_count = COALESCE(open_count, 0) + 1, last_opened_at = CURRENT_TIMESTAMP WHERE LOWER(TRIM(email)) = LOWER(?)",
          [email]
        ).catch(() => {});
        ...
        await runSql(
          "INSERT INTO email_opens (email, month, template_type, source, user_agent, ip_hash) VALUES (?, ?, ?, 'pixel', ?, ?)",
          [email, month, templateType, userAgent, ipHash]
        ).catch(() => {});
      ...
      console.error("Error in track_open:", err);
      return {
        status: 200,
        buffer: GIF_1X1,
        headers: {
  ...
  if (action === "test_email") {
    ...
    return {
      ...
      json: {
        ok: false,
        error: "All test emails failed to dispatch.",
        details: results
      }
    };
  }
    ...
    try {
      const payload = await cache.getOrSet("pending_emails_summary", async () => {
        ...
        const [missionaries, drips, powerState] = await Promise.all([
          runSql("SELECT email, name, last_name, first_name, full_name, cohort, batch_month, months_sent, max_months, psid, points, referral_code, is_active, status, last_sent_at, next_send_date, last_opened_at, open_count FROM missionaries ORDER BY is_prelisted DESC, name ASC").catch(() => []),
        ...
        const processedMissionaries = (missionaries || []).map(m => {
          ...
          const openCount = Number(m.open_count) || 0;
          const lastOpenedAt = m.last_opened_at || null;

          if (monthsSent > 0 || lastSentAt) {
            totalSentRecipients++;
          }
          if (openCount > 0 || lastOpenedAt) {
            totalOpenedCount++;
          }
          ...
          if (!isCompleted) {
            ...
            if (monthsSent === 0 || !lastSentAt) {
              if (currentMissionMonth <= 0) {
                ...
                if (m.next_send_date && m.next_send_date.slice(0, 7) === todayPhtIso.slice(0, 7)) {
                  estimatedNextDate = m.next_send_date.slice(0, 10);
                } else {
              ...
              if (monthsSent < currentMissionMonth) {
                estimatedNextDate = todayPhtIso;
              } else if (lastSentAt) {
                try {
          ...
          const isEligibleForDispatch = !isCompleted && currentMissionMonth > 0 && monthsSent < currentMissionMonth;
        ...
        const openRatePct = totalSentRecipients > 0 
          ? Math.min(100, Math.round((totalOpenedCount / totalSentRecipients) * 100))
          : (processedMissionaries.length > 0 ? Math.round((totalOpenedCount / processedMissionaries.length) * 100) : 0);
    ...
    try {
      ...
      for (const ev of rawEvents) {
        ...
        if (["hard_bounce", "soft_bounce", "blocked", "invalid_email"].includes(eventType)) {
          ...
          processed++;
        } else if (["opened", "unique_opened", "open"].includes(eventType)) {
          await runSql("UPDATE missionaries SET open_count = COALESCE(open_count, 0) + 1, last_opened_at = CURRENT_TIMESTAMP WHERE LOWER(email) = ?", [email]);
          await runSql("INSERT INTO email_opens (email, month, template_type, source, user_agent, ip_hash) VALUES (?, 0, 'brevo_event', 'brevo_webhook', ?, ?)", [
            email,
            (ev.user_agent || ev.agent || 'Brevo Webhook').slice(0, 255),
            (ev.ip || '').slice(0, 16)
          ]).catch(() => {});
          processed++;
        }
