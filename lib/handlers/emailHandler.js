...
export async function handleEmailAction(action, req, bodyData) {
  if (action === "track_open" || action === "email_open" || action === "open_pixel") {
    try {
      let email = '';
      const rawE = req.query?.e || bodyData.e || '';
      if (rawE) {
        try {
          email = Buffer.from(rawE, 'base64url').toString('utf8');
        } catch (_) {
          email = rawE;
        }
      }
      if (!email || !email.includes('@')) {
        email = (req.query?.email || bodyData.email || rawE || '').trim();
      }
      ...
      if (email && email.includes('@')) {
        await runSql(
          "UPDATE missionaries SET open_count = COALESCE(open_count, 0) + 1, last_opened_at = CURRENT_TIMESTAMP WHERE LOWER(email) = ?",
          [email]
        ).catch(() => {});
