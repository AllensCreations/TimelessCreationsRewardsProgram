...
export async function handleEmailAction(action, req, bodyData) {
  if (action === "brevo_webhook") {
    ...
    for (const evt of rawEvents) {
      ...
      if (email) {
        await runSql(`
          INSERT INTO email_events (email, event_type, subject, message_id, ip)
          VALUES (?, ?, ?, ?, ?)
        `, [email, eventType, subject, messageId, ip]).catch(() => {});

        await runSql(`
          INSERT INTO system_logs (level, message)
          VALUES ('BREVO', ?)
        `, [`[BREVO_${eventType.toUpperCase()}] ${email} - ${subject || 'Email Notification'}`]).catch(() => {});
