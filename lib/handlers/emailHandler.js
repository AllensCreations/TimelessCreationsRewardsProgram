...
export async function handleEmailAction(action, req, bodyData) {
    ...
    try {
      const payload = await cache.getOrSet("pending_emails_summary", async () => {
        ...
        const processedMissionaries = (missionaries || []).map(m => {
          ...
          if (!isCompleted) {
            if (monthsSent === 0 || !lastSentAt) {
              ...
              if (currentMissionMonth <= 0) {
                ...
                // 1st dispatch MUST occur in their 1st Month (e.g. October 2026), never in their arrival month.
                if (m.next_send_date && m.next_send_date.slice(0, 7) >= firstMonthPrefix) {
                  estimatedNextDate = m.next_send_date.slice(0, 10);
                } else {
                  estimatedNextDate = getFirstDispatchDate(m.batch_month || 'August 2026', phtNow);
                  // Self-heal stale next_send_date in DB
                ...
                // 1st Month has arrived or passed
                if (m.next_send_date && m.next_send_date.slice(0, 7) >= firstMonthPrefix) {
                  estimatedNextDate = m.next_send_date.slice(0, 10);
                } else {
          ...
          let targetCalMonth = currentCalMonth;
          if (monthsSent === 0) {
            targetCalMonth = batchInfo.firstMonthNum;
            if (estimatedNextDate && /^\d{4}-\d{2}-\d{2}/.test(estimatedNextDate)) {
              const parsedM = parseInt(estimatedNextDate.slice(5, 7), 10);
              if (!isNaN(parsedM) && parsedM >= batchInfo.firstMonthNum) {
                targetCalMonth = parsedM;
              }
            }
          } else if (estimatedNextDate && /^\d{4}-\d{2}-\d{2}/.test(estimatedNextDate)) {
            const parsedM = parseInt(estimatedNextDate.slice(5, 7), 10);
            if (!isNaN(parsedM) && parsedM >= 1 && parsedM <= 12) {
              targetCalMonth = parsedM;
            }
          } else {
            targetCalMonth = ((batchInfo.firstMonthNum - 1 + monthsSent) % 12) + 1;
          }

          let nextDripInfo = null;
          if (tenureMonthNum) {
            const dripRow = dripMap[targetCalMonth] || dripMap[((targetCalMonth - 1) % 12) + 1] || {};
            const monthLabel = getCalendarMonthLabel(targetCalMonth);
            ...
            nextDripInfo = {
              month_num: targetCalMonth,
              tenure_month: tenureMonthNum,
              calendar_month_label: monthLabel,
              display_label: `${monthLabel} (M${tenureMonthNum})`,
              subject: subject,
    ...
    try {
      ...
      if (email) {
        ...
        if (mRows && mRows.length > 0) {
          ...
          if (!customMonth) {
            if (m.next_send_date && /^\d{4}-\d{2}-\d{2}/.test(m.next_send_date)) {
              const nextMonth = parseInt(m.next_send_date.slice(5, 7), 10);
              if (!isNaN(nextMonth) && nextMonth >= 1 && nextMonth <= 12) {
                monthToRender = nextMonth;
              } else {
                const bInfo = getFirstMonthInfo(m.batch_month || 'August 2026');
                monthToRender = ((bInfo.firstMonthNum - 1 + Number(m.months_sent || 0)) % 12) + 1;
              }
            } else {
              const bInfo = getFirstMonthInfo(m.batch_month || 'August 2026');
              monthToRender = ((bInfo.firstMonthNum - 1 + Number(m.months_sent || 0)) % 12) + 1;
      ...
      return {
        ...
        json: {
          ...
          display_label: `${monthLabel} (M${tenureMonth})`,
  ...
  if (action === "dispatch_single_pending") {
    try {
      ...
      const batchInfo = getFirstMonthInfo(m.batch_month || 'August 2026');
      const defaultMonth = ((batchInfo.firstMonthNum - 1 + Number(m.months_sent || 0)) % 12) + 1;

      const targetCalMonth = Number(bodyData.month || req.query?.month) || defaultMonth;
