...
export async function handleEmailAction(action, req, bodyData) {
  ...
  if (action === "preview_pending_email" || action === "preview_email" || action === "preview_drip" || action === "get_email_preview" || action === "render_preview") {
    try {
      ...
      if (email) {
        const mRows = await runSql("SELECT name, cohort, months_sent, max_months, points, next_send_date FROM missionaries WHERE LOWER(email) = ?", [email]).catch(() => []);
  ...
  if (action === "dispatch_single_pending") {
    try {
      ...
      const targetCalMonth = Number(bodyData.month || req.query?.month) || currentCalMonth;
        ...
        await Promise.all(chunk.map(async (m) => {
          ...
          try {
            ...
            if (result?.ok) {
              ...
              sentCount++;
              dispatchedList.push({ email: m.email, name: m.name, month: currentCalMonth, tenure: tenureMonth });
            } else {
