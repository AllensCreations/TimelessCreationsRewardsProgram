...
export async function handleEmailAction(action, req, bodyData) {
    ...
    try {
      const payload = await cache.getOrSet("pending_emails_summary", async () => {
        ...
        const processedMissionaries = (missionaries || []).map(m => {
          ...
          let nextDripMonthInfo = null;
          if (tenureMonthNum) {
            nextDripMonthInfo = getMissionMonthInfo(m.batch_month || 'August 2026', tenureMonthNum);
            targetCalMonth = nextDripMonthInfo.monthNum;
          }

          let nextDripInfo = null;
          if (tenureMonthNum && nextDripMonthInfo) {
            const dripRow = dripMap[targetCalMonth] || dripMap[((targetCalMonth - 1) % 12) + 1] || {};
            const monthLabel = nextDripMonthInfo.monthName;
            const monthYearDisplay = nextDripMonthInfo.display;
            ...
            nextDripInfo = {
              ...
              calendar_year: nextDripMonthInfo.year,
              month_year: monthYearDisplay,
              display_label: `${monthYearDisplay} (M${tenureMonthNum})`,
              subject: subject,
    ...
    try {
      ...
      if (email) {
        ...
        if (mRows && mRows.length > 0) {
          ...
          if (!customMonth) {
            const mInfo = getMissionMonthInfo(m.batch_month || 'August 2026', tenureMonth);
            monthToRender = mInfo.monthNum;
          }
        }
      }
  ...
  if (action === "dispatch_single_pending") {
    try {
      ...
      const mInfo = getMissionMonthInfo(m.batch_month || 'August 2026', tenureMonth);
      const defaultMonth = mInfo.monthNum;
      ...
      for (let i = 0; i < dueMissionaries.length; i += CONCURRENCY_CHUNK_SIZE) {
        ...
        await Promise.all(chunk.map(async (m) => {
          ...
          try {
            ...
            const recipientName = m.name || (isSister ? 'Sister' : 'Elder');
            const mInfo = getMissionMonthInfo(m.batch_month || 'August 2026', tenureMonth);
            const targetCalMonth = mInfo.monthNum;
