...
export async function handleInvoiceAction(action, req, bodyData) {
  if (action === "get_invoices") {
    const invoices = await cache.getOrSet("invoices_list", async () => {
      const res = await runSql("SELECT * FROM cash_invoices ORDER BY created_at DESC LIMIT 100");
      return res || [];
    }, 20000, ["invoices"]);
  ...
  if (action === "update_invoice_status") {
    ...
    const existing = (await runSql("SELECT * FROM cash_invoices WHERE invoice_id = ?", [invoice_id]))[0];
    ...
    if (cleanStatus === 'DELIVERED') {
      await runSql("UPDATE cash_invoices SET status = ?, delivered_at = COALESCE(delivered_at, CURRENT_TIMESTAMP) WHERE invoice_id = ?", [cleanStatus, invoice_id]);
    } else {
