...
export async function handleMissionaryAction(action, req, bodyData) {
  ...
  if (action === "update_missionary") {
    ...
    await runSql(`
      ...
      WHERE LOWER(email) = ?
    `, [newEmail, name || '', isSister ? 'sister' : 'elder', finalBatch, Number(points) || 0, status || 'active', referral_code || '', maxMonths, targetEmail]);
  ...
  if (action === "push_missionaries" || action === "get_push_history") {
    ...
    if (req.method === "POST") {
      ...
      for (const item of entries) {
        ...
        if (existing) {
          await runSql(
            "UPDATE missionaries SET name = ?, last_name = ?, first_name = ?, full_name = ?, cohort = ?, batch_month = ?, max_months = ?, is_prelisted = 1, next_send_date = CASE WHEN (months_sent = 0 OR months_sent IS NULL) AND last_sent_at IS NULL THEN ? ELSE next_send_date END WHERE LOWER(email) = ?",
            [titleName, lastName, firstName, fullName, cohort, batchMonth, maxMonths, initialSendDate, email]
          );
          added++;
          continue;
        }
        ...
        await runSql(
          "INSERT INTO missionaries (email, name, last_name, first_name, full_name, cohort, batch_month, referral_code, max_months, points, status, is_prelisted, next_send_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 'active', 1, ?)",
          [email, titleName, lastName, firstName, fullName, cohort, batchMonth, refCode, maxMonths, initialSendDate]
        );
        added++;
      }
