...
export async function resolveBotMaintenanceState() {
  try {
    const [rowSettings, rowConfig] = await Promise.all([
      runSql("SELECT value FROM system_settings WHERE key = 'bot_maintenance'").catch(() => []),
      runSql("SELECT value FROM system_config WHERE key = 'bot_maintenance'").catch(() => [])
    ]);
    const val = rowSettings?.[0]?.value || rowConfig?.[0]?.value;
...
export async function persistBotMaintenanceState(newState) {
  ...
  try {
    await Promise.all([
      ...
      `, [cleanState]).catch(() => {}),
      runSql(`
        INSERT INTO system_config (key, value) VALUES ('bot_maintenance', ?)
        ON CONFLICT(key) DO UPDATE SET value = excluded.value
      `, [cleanState]).catch(() => {}),
      runSql("INSERT INTO system_logs (level, message) VALUES ('INFO', ?)", [`Messenger bot maintenance mode switched to ${cleanState}`]).catch(() => {})
    ]);
  } catch (err) {
