...
export function readLocalVersion() {
  const defaults = {
    version: "2.49.0",
    version_code: 62,
    deployment_id: "deploy_20260911_v2_49",
    build_timestamp: "2026-09-11T07:00:00Z",
    changelog: "v2.49.0 (Build 62)"
  };
  ...
  if (fs.existsSync(pkgPath)) {
    try {
      ...
      if (pkg && pkg.version) {
        const cleanVer = String(pkg.version).replace(/^v/i, '');
        const vCode = Number(pkg.versionCode || pkg.version_code) || defaults.version_code;
        return {
          version: cleanVer,
          version_code: vCode,
          deployment_id: `deploy_${cleanVer.replace(/\./g, '_')}`,
          build_timestamp: new Date().toISOString(),
          changelog: `v${cleanVer} (Build ${vCode})`
        };
      }
    } catch (_) {}
  }
...
export async function fetchGitHubRelease(force = false) {
  ...
  if (releaseData) {
    ...
    cachedReleaseInfo = {
      ...
      github_apk_url: `https://github.com/${repo}/raw/main/public/TimelessRewards.apk`,
...
export async function handleSystemAction(action, req, bodyData) {
  switch (action) {
    ...
    case "check_update": {
      ...
      const currentVer = String(clientVerParam || local.version || "2.1.0").replace(/^v/i, '');
      const currentCode = clientCodeParam ? Number(clientCodeParam) : (Number(local.version_code) || 13);

      const remoteVer = String(release.version || currentVer).replace(/^v/i, '');
      const remoteCode = Number(release.version_code) || currentCode;

      const isSame = (remoteCode === currentCode) && (remoteVer === currentVer);
      const isOlderOrEqual = (remoteCode <= currentCode) && !isNewerSemVer(remoteVer, currentVer);
      const effectiveSame = isSame || isOlderOrEqual;
      const hasUpdate = !effectiveSame && ((remoteCode > currentCode) || isNewerSemVer(remoteVer, currentVer));

      let comparisonStatus = effectiveSame ? "same" : (hasUpdate ? "update_available" : "different");
      let statusLabel = effectiveSame ? "Identical (Up to date)" : (hasUpdate ? "New Update Available" : "Different Build");
      let statusMessage = effectiveSame
        ? `✓ Installed APK matches the latest GitHub Release (${release.tag_name || ('v' + remoteVer)} • Build ${remoteCode}).`
        : (hasUpdate
            ? `🚀 New update detected in Release Link: ${release.tag_name || ('v' + remoteVer)} (Build ${remoteCode}). Current is v${currentVer} (Build ${currentCode}).`
            : `Notice: Installed version is v${currentVer} (Build ${currentCode}) and release is ${release.tag_name || ('v' + remoteVer)} (Build ${remoteCode}).`);
    ...
    case "get_email_events": {
      const limit = Math.min(Number(req.query?.limit || bodyData.limit) || 25, 100);
      const events = await runSql(`
        SELECT e.id, e.email, e.event_type, e.subject, e.message_id, e.ip, e.created_at, m.name, m.cohort
        FROM system_logs e
        LEFT JOIN missionaries m ON LOWER(e.email) = LOWER(m.email)
        WHERE e.source = 'EMAIL_BREVO'
        ORDER BY e.id DESC
        LIMIT ?
      `, [limit]).catch(() => []);
      return { status: 200, json: { ok: true, events: events || [] } };
    }
    ...
    case "": {
      ...
      const [
        ...
        yearlyRows, powerRow, recentEvents, openCountRows, deliveredCountRows,
        remainingMonth
      ] = await Promise.all([
        ...
        runSql("SELECT COUNT(*) as count FROM orders WHERE type = 'REWARD' OR type IS NULL").catch(() => [{ count: 0 }]),
        runSql("SELECT COUNT(*) as count FROM orders WHERE (type = 'REWARD' OR type IS NULL) AND UPPER(status) = 'PENDING'").catch(() => [{ count: 0 }]),
        ...
        runSql("SELECT order_id, name, item, points_cost, status, created_at FROM orders WHERE type = 'REWARD' OR type IS NULL ORDER BY created_at DESC LIMIT 5").catch(() => []),
        runSql("SELECT id, level, message, created_at FROM system_logs WHERE source = 'SYSTEM' OR source IS NULL ORDER BY id DESC LIMIT 50").catch(() => []),
        runSql("SELECT COUNT(*) as count FROM missionaries WHERE (substr(last_sent_at, 1, 10) = ? OR date(datetime(last_sent_at, '+8 hours')) = ?)", [todayIso, todayIso]).catch(() => [{ count: 0 }]),
        runSql("SELECT COUNT(*) as count FROM missionaries WHERE (substr(last_sent_at, 1, 7) = ? OR strftime('%Y-%m', datetime(last_sent_at, '+8 hours')) = ?)", [monthIso, monthIso]).catch(() => [{ count: 0 }]),
        ...
        null,
        runSql(`
          SELECT e.id, e.email, e.event_type, e.subject, e.message_id, e.created_at, m.name, m.cohort
          FROM system_logs e
          LEFT JOIN missionaries m ON LOWER(e.email) = LOWER(m.email)
          WHERE e.source = 'EMAIL_BREVO'
          ORDER BY e.id DESC
        ...
        runSql(`
          ...
          WHERE source = 'EMAIL_BREVO'
            AND event_type IN ('opened', 'unique_opened')
            AND created_at LIKE ?
        `, [monthIso + "%"]).catch(() => [{ cnt: 0 }]),
        runSql(`
          SELECT COUNT(*) as cnt FROM system_logs 
          WHERE source = 'EMAIL_BREVO'
        ...
        runSql(`
          ...
          FROM missionaries 
          WHERE status = 'active'
            AND (months_sent < max_months OR months_sent IS NULL)
            AND (substr(next_send_date, 1, 7) <= ? OR next_send_date IS NULL)
            AND (last_sent_at IS NULL OR (substr(last_sent_at, 1, 7) != ? AND strftime('%Y-%m', datetime(last_sent_at, '+8 hours')) != ?))
        `, [monthIso, monthIso, monthIso]).catch(() => [{ count: 0 }])
      ]);

      const opensMonth = Number(openCountRows?.[0]?.cnt || 0);
      const deliveredMonth = Number(deliveredCountRows?.[0]?.cnt || 0);
      const dispatchesMonth = Number(monthSent?.[0]?.count || 0);
      const baseDelivery = Math.max(deliveredMonth, dispatchesMonth);
      const openRate = baseDelivery > 0 ? Math.min(100, Math.round((opensMonth / baseDelivery) * 100)) : 0;
      ...
      const prevYr = String(phtDate.getFullYear() - 1);
      yearlyHistory[currentYr] = {};
      yearlyHistory[prevYr] = {};
      ...
      // Check if custom yearly dispatch history is stored in system_settings
      try {
        const customRows = await runSql("SELECT value FROM system_settings WHERE key = 'yearly_dispatch_history' LIMIT 1").catch(() => []);
        if (customRows[0]?.value) {
          const parsed = JSON.parse(customRows[0].value);
          if (parsed && typeof parsed === 'object') {
            for (const [y, mData] of Object.entries(parsed)) {
              if (!yearlyHistory[y]) yearlyHistory[y] = {};
              for (const [m, v] of Object.entries(mData)) {
                yearlyHistory[y][parseInt(m, 10)] = Number(v) || 0;
              }
            }
          }
        }
      ...
      return {
        ...
        json: {
          ...
          stats: {
            ...
            remaining_this_month: remainingMonth[0]?.count || 0,
            opens_month: opensMonth,
            delivered_month: deliveredMonth,
            open_rate: openRate,
            hypothetical_monthly_target: monthlyHypotheticalTarget
          },
          remaining_this_month: remainingMonth[0]?.count || 0,
          email_events: recentEvents || [],
          open_rate: openRate,
          opens_month: opensMonth,
          delivered_month: deliveredMonth,
