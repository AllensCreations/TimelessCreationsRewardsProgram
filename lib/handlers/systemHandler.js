  ...
  switch (action) {
    ...
    case "toggle_power": {
      ...
      try {
        ...
        console.error("Error updating power state in DB:", err);
      }
      return { status: 200, json: { ok: true, state, power_state: state, is_online: state === "ONLINE" } };
    }
    ...
    case "": {
      ...
      const [
        ...
        runSql("SELECT value FROM system_settings WHERE key = 'power_state'").catch(() => [{ value: 'ONLINE' }])
      ]);

      const currentPowerState = (powerRow[0]?.value || 'ONLINE').toUpperCase();
