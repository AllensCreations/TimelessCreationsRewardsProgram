  ...
  switch (action) {
    ...
    case "": {
      ...
      const [
        ...
        runSql("SELECT strftime('%Y', created_at) as yr, strftime('%m', created_at) as mo, COUNT(*) as cnt FROM system_logs WHERE (level = 'DISPATCH' OR message LIKE '%[EMAIL_DISPATCH]%' OR message LIKE '%sent to%') GROUP BY yr, mo ORDER BY yr DESC, mo ASC").catch(() => [])
      ]);
      ...
      // Ensure current month from missionaries is reflected
