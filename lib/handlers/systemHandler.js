...
function readLocalVersion() {
  const defaults = {
    version: "1.9.0",
    version_code: 11,
    deployment_id: "deploy_20260904_v1_9",
    build_timestamp: "2026-09-04T12:17:21Z",
    changelog: "v1.9.0: Automated CI deployment build 11"
  };
...
export async function fetchGitHubRelease(force = false) {
  ...
  try {
    const res = await fetch(releaseApiUrl, {
      headers: {
        'User-Agent': 'TCRP-Update-Checker/1.9.0',
...
export async function handleSystemAction(action, req, bodyData) {
  switch (action) {
    ...
    case "check_update": {
      ...
      const currentVer = clientVerParam || local.version || "1.9.0";
      const currentCode = clientCodeParam ? Number(clientCodeParam) : (Number(local.version_code) || 11);
