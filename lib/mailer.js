...
import { getCdnImageUrl, JSDELIVR_BASE } from './cdn.js';
    ...
    <tr>
      <td align="center">
        ...
        <table class="email-container" width="100%" border="0" cellspacing="0" cellpadding="0" style="max-width:440px; background-color:#ffffff; border:1px solid #e0d6bc; box-shadow:0 10px 30px rgba(0,0,0,0.08); overflow:hidden;">
          ...
          <tr>
            <td>
              <img src="{{TEMPLE_IMG}}" alt="Temple" style="width:100%; max-width:100%; height:auto; min-height:140px; max-height:220px; object-fit:cover; display:block; border:0;">
            </td>
          </tr>
 ...
 */
export function formatMetricK(num) {
  if (num === null || num === undefined || num === '') return "0 Sold";
  const str = String(num).trim();
  if (str.toLowerCase().includes('k') || str.toLowerCase().includes('m')) return str;
  const digits = parseInt(str.replace(/\D/g, ''), 10);
  if (isNaN(digits)) return str;
  if (digits >= 1000000) {
    return `${(digits / 1000000).toFixed(1).replace('.0', '')}M+ Sold`;
  }
  if (digits >= 1000) {
    return `${(digits / 1000).toFixed(1).replace('.0', '')}k+ Sold`;
  }
  return `${digits} Sold`;
}

/**
 * Extract design and color tokens from the custom template's <style> block and markup
 * so that {{REWARD_SECTION_HTML}} and {{TOP_PRODUCTS_HTML}} adapt dynamically to the custom template!
 */
export function extractTemplateTheme(templateHtml = '') {
  const theme = {
    ...
    btnBg: "#3b2f2f",
    btnColor: "#ffffff",
    secondaryBadgeBg: "#555566",
    secondaryBadgeColor: "#ffffff"
  };

  if (!templateHtml) return theme;
  if (typeof templateHtml === 'object') return { ...theme, ...templateHtml };
  if (typeof templateHtml !== 'string') return theme;

  const styleMatch = templateHtml.match(/<style[^>]*>([\s\S]*?)<\/style>/i);
  const styleContent = styleMatch ? styleMatch[1] : '';

  // 1. Check custom reward classes defined in <style>
  const customCardBg = styleContent.match(/\.reward-section-card\s*\{[^}]*background(?:-color)?\s*:\s*([^;!}]+)/i);
  if (customCardBg) theme.cardBg = customCardBg[1].trim();

  const customCardBorder = styleContent.match(/\.reward-section-card\s*\{[^}]*border(?:-[a-z]+)?\s*:\s*[^;!}]*?#([a-fA-F0-9]{3,6})/i);
  if (customCardBorder) theme.cardBorder = '#' + customCardBorder[1].trim();

  const customRewardTitle = styleContent.match(/\.reward-title\s*\{[^}]*color\s*:\s*([^;!}]+)/i);
  if (customRewardTitle) theme.textColor = customRewardTitle[1].trim();

  // 2. Primary Button background-color
  const btnBgMatch = styleContent.match(/\.btn-primary\s*\{[^}]*background(?:-color)?\s*:\s*([^;!}]+)/i);
  if (btnBgMatch) {
    theme.btnBg = btnBgMatch[1].trim();
  } else {
    // Check for inline button style or custom button colors in HTML body
    const inlineBtnMatch = templateHtml.match(/background-color\s*:\s*(#[a-fA-F0-9]{3,6})[^"']*;\s*color\s*:\s*#[a-fA-F0-9]{3,6}\s*!important[^"']*text-transform\s*:\s*uppercase/i)
      || templateHtml.match(/background-color\s*:\s*(#[a-fA-F0-9]{3,6})[^"']*;\s*color\s*:\s*#[a-fA-F0-9]{3,6}\s*!important/i);
    if (inlineBtnMatch) {
      theme.btnBg = inlineBtnMatch[1].trim();
    }
  }

  // 3. Primary Button text color
  const btnColorMatch = styleContent.match(/\.btn-primary\s*\{[^}]*color\s*:\s*([^;!}]+)/i);
  if (btnColorMatch) {
    theme.btnColor = btnColorMatch[1].replace('!important', '').trim();
  }

  // 4. Font family from body or email containers
  const fontMatch = styleContent.match(/(?:body|\.email-container|\.email-wrapper)\s*\{[^}]*font-family\s*:\s*([^;!}]+)/i);
  if (fontMatch) {
    theme.fontFamily = fontMatch[1].trim();
  }

  // 5. Card background from .info-card or inline cards
  if (!customCardBg) {
    const infoCardBgMatch = styleContent.match(/\.info-card\s*\{[^}]*background(?:-color)?\s*:\s*([^;!}]+)/i);
    if (infoCardBgMatch) {
      theme.cardBg = infoCardBgMatch[1].trim();
    } else {
      const inlineCardBg = templateHtml.match(/background-color\s*:\s*(#[a-fA-F0-9]{3,6})[^"']*;\s*border\s*:\s*1px\s*solid\s*(#[a-fA-F0-9]{3,6})/i);
      if (inlineCardBg) {
        theme.cardBg = inlineCardBg[1].trim();
        if (!customCardBorder) theme.cardBorder = inlineCardBg[2].trim();
      }
    }
  }

  // 6. Card border from .info-card or .email-container
  if (!customCardBorder) {
    const borderMatch = styleContent.match(/(?:\.info-card|\.email-container)\s*\{[^}]*border(?:-[a-z]+)?\s*:\s*[^;!}]*?#([a-fA-F0-9]{3,6})/i);
    if (borderMatch) {
      theme.cardBorder = '#' + borderMatch[1].trim();
    }
  }

  // 7. Accent color from .hero-tag or header
  const accentMatch = styleContent.match(/(?:\.hero-tag|\.brand-header)\s*\{[^}]*background(?:-color)?\s*:\s*([^;!}]+)/i);
  if (accentMatch) {
    theme.accentColor = accentMatch[1].trim();
  } else {
    theme.accentColor = theme.btnBg;
  }

  // 8. Body text color
  const textMatch = styleContent.match(/(?:body|\.main-content)\s*\{[^}]*color\s*:\s*([^;!}]+)/i);
  if (textMatch) {
    theme.textColor = textMatch[1].trim();
  }
 ...
 */
export function buildTopProductsHtml(dripData = {}, themeInput = null) {
  const isExplicitlyDisabled = dripData.highlight_enabled === false || (dripData.highlight_label || "").trim() === "__DISABLED__";
  if (isExplicitlyDisabled) return "";

  const theme = extractTemplateTheme(themeInput);

  const p1Label = (dripData.highlight_label || dripData.highlight_product_name || dripData.product_1_name || "").trim();
  if (p1Label === "__DISABLED__") return "";

  const p1Name = p1Label || "Custom Wooden Nametag";
  const p1Img = getCdnImageUrl(dripData.highlight_img || dripData.highlight_product_img || dripData.product_1_img || "https://lh3.googleusercontent.com/u/0/d/1F7Yb0OzuCmPO2LyZ0cMoaTM4d4rs5RFE", "product_1.jpg");
  const p1Sold = formatMetricK(dripData.highlight_sold_1 || dripData.highlight_product_sold || dripData.product_1_sold || "1.5k+ Sold");

  const p2Label = (dripData.highlight_label_2 || dripData.highlight_product_2_name || dripData.product_2_name || "").trim();
  const p2Name = p2Label || (p1Label ? "" : "LDS Temple Keychain");
  const p2Img = getCdnImageUrl(dripData.highlight_img_2 || dripData.highlight_product_2_img || dripData.product_2_img || "https://lh3.googleusercontent.com/u/0/d/101jY71PjxCwiuNznTgn7Xyc0HoXwB3WQ", "product_2.jpg");
  const p2Sold = formatMetricK(dripData.highlight_sold_2 || dripData.highlight_product_2_sold || dripData.product_2_sold || "950+ Sold");

  if (!p1Name && !p2Name) return "";

  if (p1Name && !p2Name) {
    return `
      <div class="top-products-section" style="margin: 20px 0; padding: 16px 12px; background: ${theme.cardBg}; border: 1px solid ${theme.cardBorder}; border-radius: 6px; text-align: center;">
        <span class="top-product-badge" style="display: inline-block; background: ${theme.btnBg}; color: ${theme.btnColor}; font-family: ${theme.fontFamily}; font-size: 8.5px; letter-spacing: 1.5px; text-transform: uppercase; font-weight: bold; padding: 3px 8px; border-radius: 3px; margin-bottom: 8px;">
          🥇 #1 TOP BESTSELLER
        </span>
        <img src="${p1Img}" style="width: 100%; max-width: 130px; aspect-ratio: 1/1; object-fit: cover; border-radius: 4px; border: 1px solid ${theme.cardBorder}; display: block; margin: 0 auto 6px auto;" alt="${p1Name}">
        <div style="font-family: ${theme.fontFamily}; font-size: 11.5px; font-weight: bold; color: ${theme.textColor};">${p1Name}</div>
        <div style="font-size: 10px; color: ${theme.accentColor}; font-weight: bold; margin-top: 2px;">🔥 ${formatMetricK(p1Sold)}</div>
      </div>
    `;
  }
  ...
  if (!p1Name && p2Name) {
    return `
      <div class="top-products-section" style="margin: 20px 0; padding: 16px 12px; background: ${theme.cardBg}; border: 1px solid ${theme.cardBorder}; border-radius: 6px; text-align: center;">
        ...
        <img src="${p2Img}" style="width: 100%; max-width: 130px; aspect-ratio: 1/1; object-fit: cover; border-radius: 4px; border: 1px solid ${theme.cardBorder}; display: block; margin: 0 auto 6px auto;" alt="${p2Name}">
        <div style="font-family: ${theme.fontFamily}; font-size: 11.5px; font-weight: bold; color: ${theme.textColor};">${p2Name}</div>
        <div style="font-size: 10px; color: ${theme.accentColor}; font-weight: bold; margin-top: 2px;">🔥 ${formatMetricK(p2Sold)}</div>
      </div>
    `;
  }

  return `
    <div class="top-products-section" style="margin: 20px 0; padding: 16px 12px; background: ${theme.cardBg}; border: 1px solid ${theme.cardBorder}; border-radius: 6px;">
      <div class="top-products-header" style="font-family: ${theme.fontFamily}; font-size: 9px; letter-spacing: 2px; text-transform: uppercase; color: ${theme.btnBg}; font-weight: bold; text-align: center; margin-bottom: 12px;">
        ⭐ Top Products of the Month
      </div>
      <table width="100%" border="0" cellspacing="0" cellpadding="0" style="table-layout: fixed;">
        <tr>
          <!-- TOP 1 (Column 1) -->
          <td align="center" width="48%" valign="top" class="top-product-card" style="background: #ffffff; border: 1.5px solid ${theme.cardBorder}; border-radius: 6px; padding: 12px 8px; box-shadow: 0 4px 10px rgba(0,0,0,0.04);">
            <span class="top-product-badge" style="display: inline-block; background: ${theme.btnBg}; color: ${theme.btnColor}; font-family: ${theme.fontFamily}; font-size: 7.5px; letter-spacing: 1px; text-transform: uppercase; font-weight: bold; padding: 2px 6px; border-radius: 3px; margin-bottom: 6px;">
              🥇 #1 TOP SELLER
            </span>
            <img src="${p1Img}" style="width: 100%; max-width: 110px; aspect-ratio: 1/1; object-fit: cover; border-radius: 4px; border: 1px solid ${theme.cardBorder}; display: block; margin: 0 auto 6px auto;" alt="${p1Name}">
            <div style="font-family: ${theme.fontFamily}; font-size: 11px; font-weight: bold; color: ${theme.textColor}; line-height: 1.2;">${p1Name}</div>
            <div style="font-size: 9.5px; color: ${theme.accentColor}; font-weight: bold; margin-top: 3px;">🔥 ${formatMetricK(p1Sold)}</div>
          </td>
          <td width="4%"></td>
          <!-- TOP 2 (Column 2) -->
          <td align="center" width="48%" valign="top" class="top-product-card secondary" style="background: #ffffff; border: 1px dashed ${theme.cardBorder}; border-radius: 6px; padding: 12px 8px;">
            <span class="top-product-badge secondary" style="display: inline-block; background: ${theme.secondaryBadgeBg}; color: ${theme.secondaryBadgeColor}; font-family: ${theme.fontFamily}; font-size: 7.5px; letter-spacing: 1px; text-transform: uppercase; font-weight: bold; padding: 2px 6px; border-radius: 3px; margin-bottom: 6px;">
              🥈 #2 POPULAR
            </span>
            <img src="${p2Img}" style="width: 100%; max-width: 95px; aspect-ratio: 1/1; object-fit: cover; border-radius: 4px; border: 1px solid ${theme.cardBorder}; display: block;" alt="${p2Name}">
            <div style="font-family: ${theme.fontFamily}; font-size: 10.5px; font-weight: bold; color: ${theme.textColor}; line-height: 1.2;">${p2Name}</div>
            <div style="font-size: 9px; color: ${theme.accentColor}; font-weight: bold; margin-top: 3px;">⭐ ${formatMetricK(p2Sold)}</div>
          </td>
        </tr>
      </table>
    </div>
...
/**
 * Component Breakdown Builder for Top Products
 */
export function buildTopProductsComponents(dripData = {}, themeInput = null) {
  const isExplicitlyDisabled = dripData.highlight_enabled === false || (dripData.highlight_label || "").trim() === "__DISABLED__";
  ...
  const combinedHtml = buildTopProductsHtml(dripData, theme);
  ...
  return {
    ...
    p2Img,
    p2Sold,
    display: combinedHtml ? "display:block" : "display:none",
    combinedHtml
  };
}

/**
 * Component Breakdown Builder for Rewards & Promos
 */
export function buildRewardSectionComponents(userPoints = 2, catalogProducts = [], activePromo = null, themeInput = null) {
  const theme = extractTemplateTheme(themeInput);
  const pts = Number(userPoints) || 0;
  const validProducts = Array.isArray(catalogProducts) ? catalogProducts : [];
  const affordable = validProducts.filter(p => Number(p.price) <= pts);

  let promoSnippetHtml = "";
  let promoCode = "";
  let promoPoints = "0";
  if (activePromo && activePromo.code && activePromo.code.trim() !== "") {
    promoCode = activePromo.code.trim();
    promoPoints = String(activePromo.points || 1);
    promoSnippetHtml = `
      <div class="reward-promo-box" style="margin: 0 0 14px 0; padding: 12px 14px; background-color: #ffffff; border: 1.5px dashed ${theme.cardBorder}; border-radius: 6px; text-align: center;">
        <span style="font-family: ${theme.fontFamily}; font-size: 8.5px; letter-spacing: 2px; text-transform: uppercase; color: ${theme.accentColor}; font-weight: bold; display: block; margin-bottom: 3px;">
          🎟️ Exclusive Monthly Freebie Promo
        </span>
        <div style="font-size: 12px; color: ${theme.textColor}; margin-bottom: 5px;">
          Claim <strong>+${promoPoints} Free Reward Points</strong> right now!
        </div>
        <div class="reward-promo-code" style="font-family: monospace; font-size: 13px; font-weight: bold; background: ${theme.btnBg}; color: ${theme.btnColor}; padding: 6px 14px; border-radius: 4px; display: inline-block; letter-spacing: 1px;">
          ${promoCode}
        </div>
        <div style="font-size: 9.5px; color: ${theme.mutedColor}; margin-top: 5px;">
          Send <strong>${promoCode}</strong> in Messenger to claim +${promoPoints} Free Points (or /redeem ${promoCode})
        </div>
      </div>
    `;
  }
  ...
  if (affordable.length > 0) {
    affordableHtml = `
      <div style="font-size: 11px; font-weight: bold; color: ${theme.textColor}; margin-bottom: 8px; text-align: left;">
        🎁 You can claim these reward items right now:
      </div>
      <table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-bottom: 12px;">
        ${affordable.map(item => `
          <tr>
            <td style="padding: 6px; background: #ffffff; border: 1px solid ${theme.cardBorder}; border-radius: 4px; margin-bottom: 6px;">
              <table width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr>
                  <td width="36" valign="middle">
                    <img src="${getCdnImageUrl(item.image_url || 'https://i.postimg.cc/FFdrCNqq/Untitled56-20260820115353.png')}" style="width: 32px; height: 32px; aspect-ratio: 1/1; object-fit: cover; border-radius: 3px; display: block; border: 1px solid ${theme.cardBorder};" alt="${item.name}">
                  </td>
                  <td style="padding-left: 8px; text-align: left;" valign="middle">
                    <div style="font-weight: bold; font-size: 11px; color: ${theme.textColor};">${item.name}</div>
                    <div style="font-size: 9.5px; color: ${theme.accentColor}; font-weight: bold;">⭐ ${item.price} Points</div>
                  </td>
                </tr>
              </table>
            </td>
  ...
  let goalHtml = "";
  const unreached = validProducts
    .filter(p => Number(p.price) > pts)
    .sort((a, b) => Number(a.price) - Number(b.price));

  let nearestItem = null;
  let needed = 0;
  if (unreached.length > 0) {
    nearestItem = unreached[0];
    needed = Math.max(1, Number(nearestItem.price) - pts);

    goalHtml = `
      <div class="reward-goal-card" style="background: #ffffff; border: 1px solid ${theme.cardBorder}; border-radius: 6px; padding: 12px; text-align: center; margin-bottom: 10px;">
        <span style="font-size: 9px; color: ${theme.accentColor}; font-weight: bold; letter-spacing: 1.5px; text-transform: uppercase; display: block; margin-bottom: 6px;">
          🎯 Nearest Reward Goal:
        </span>
        <img src="${getCdnImageUrl(nearestItem.image_url || 'https://i.postimg.cc/FFdrCNqq/Untitled56-20260820115353.png')}" style="width: 50px; height: 50px; aspect-ratio: 1/1; object-fit: cover; border-radius: 4px; display: block; margin: 0 auto 6px auto; border: 1px solid ${theme.cardBorder};" alt="${nearestItem.name}">
        <div style="font-weight: bold; font-size: 11.5px; color: ${theme.textColor};">${nearestItem.name} (${nearestItem.price} PTS)</div>
        <div style="font-size: 11px; color: ${theme.accentColor}; margin-top: 4px; font-weight: bold;">⚡ Only <strong>${needed} more point${needed > 1 ? 's' : ''}</strong> needed!</div>
      </div>
    `;
  }

  const combinedHtml = `
    <div class="reward-section-card" style="margin: 24px 0; padding: 18px 14px; background-color: ${theme.cardBg}; border: 1px solid ${theme.cardBorder}; border-radius: 6px; text-align: center;">
      <div class="reward-title" style="font-family: ${theme.fontFamily}; font-size: 13px; font-weight: bold; color: ${theme.btnBg}; margin-bottom: 4px;">
        🎁 Your TCRP Reward Balance
      </div>
      <p class="reward-balance-text" style="font-size: 12px; margin: 0 0 14px 0; font-family: ${theme.fontFamily}; color: ${theme.textColor};">
        You currently have <strong>${pts} Points</strong>
      </p>
      ...
      <a href="https://m.me/TimelessCreationsRP" target="_blank" class="btn-primary reward-redeem-btn" style="display: block; width: 90%; max-width: 320px; margin: 12px auto 0; padding: 12px 16px; background-color: ${theme.btnBg}; color: ${theme.btnColor} !important; text-decoration: none; font-family: ${theme.fontFamily}; font-size: 10px; text-transform: uppercase; letter-spacing: 1.5px; font-weight: bold; border-radius: 4px; box-sizing: border-box; text-align: center;">
 ...
 */
export function buildCombinedRewardSectionHtml(userPoints = 2, catalogProducts = [], activePromo = null, themeInput = null) {
  return buildRewardSectionComponents(userPoints, catalogProducts, activePromo, themeInput).combinedHtml;
}
...
export function renderMonthlyDripTemplate(dripData = {}, rewardProducts = [], activePromo = null) {
  ...
  // Extract theme from rawTemplate so dynamic blocks blend seamlessly with custom styles!
  const templateTheme = extractTemplateTheme(rawTemplate);

  const topProductsComp = buildTopProductsComponents(dripData, templateTheme);
  const isRewardsCategoryVisible = (dripData.show_rewards_category !== undefined)
    ? Boolean(dripData.show_rewards_category)
    : (dripData.drip_rewards_visible !== undefined ? Boolean(dripData.drip_rewards_visible) : true);
  const rewardComp = buildRewardSectionComponents(points, rewardProducts, activePromo, templateTheme);
  const rewardSectionHtml = isRewardsCategoryVisible ? rewardComp.combinedHtml : '';

  const defaultGrids = [
    `${JSDELIVR_BASE}/drips/grid_1.jpg`,
    `${JSDELIVR_BASE}/drips/grid_2.jpg`,
    `${JSDELIVR_BASE}/drips/grid_3.jpg`,
    `${JSDELIVR_BASE}/drips/grid_4.jpg`,
    `${JSDELIVR_BASE}/drips/grid_5.jpg`,
    `${JSDELIVR_BASE}/drips/grid_6.jpg`,
    `${JSDELIVR_BASE}/drips/grid_7.jpg`,
    `${JSDELIVR_BASE}/drips/grid_8.jpg`,
    `${JSDELIVR_BASE}/drips/grid_9.jpg`
  ];

  const replacements = {
    MONTH: monthLabel,
    TEMPLE_IMG: getCdnImageUrl(dripData.temple_img || 'https://lh3.googleusercontent.com/u/0/d/1IkagW3wWhIhfaG01mBL4wNF-1j2lP6YG', 'temple_hero.jpg'),
    NAME: name,
    MESSAGE: message,
    SCRIPTURE: scripture,
    THEME: theme,
    ...
    // Unified Macro Blocks (blended dynamically with custom template styles)
    TOP_PRODUCTS_HTML: topProductsComp.combinedHtml,
    REWARD_SECTION_HTML: rewardSectionHtml,

    // Granular Top Products tags for Custom Templates
    PRODUCT_1_NAME: topProductsComp.p1Name,
    PRODUCT_1_IMG: topProductsComp.p1Img,
    PRODUCT_1_SOLD: topProductsComp.p1Sold,
    PRODUCT_2_NAME: topProductsComp.p2Name,
    PRODUCT_2_IMG: topProductsComp.p2Img,
    PRODUCT_2_SOLD: topProductsComp.p2Sold,
    TOP_PRODUCTS_DISPLAY: topProductsComp.display,

    // Granular Rewards & Goal tags for Custom Templates
    AFFORDABLE_REWARDS_HTML: isRewardsCategoryVisible ? rewardComp.affordableHtml : '',
    AFFORDABLE_COUNT: isRewardsCategoryVisible ? String(rewardComp.affordableCount) : '0',
    NEAREST_GOAL_HTML: isRewardsCategoryVisible ? rewardComp.goalHtml : '',
    NEAREST_GOAL_NAME: (isRewardsCategoryVisible && rewardComp.nearestItem) ? rewardComp.nearestItem.name : '',
    NEAREST_GOAL_POINTS: (isRewardsCategoryVisible && rewardComp.nearestItem) ? String(rewardComp.nearestItem.price) : '',
    NEAREST_GOAL_NEEDED: (isRewardsCategoryVisible && rewardComp.nearestItem) ? String(rewardComp.needed) : '0',
    NEAREST_GOAL_IMAGE: (isRewardsCategoryVisible && rewardComp.nearestItem) ? getCdnImageUrl(rewardComp.nearestItem.image_url || 'https://i.postimg.cc/FFdrCNqq/Untitled56-20260820115353.png') : '',
    NEAREST_GOAL_IMG: (isRewardsCategoryVisible && rewardComp.nearestItem) ? getCdnImageUrl(rewardComp.nearestItem.image_url || 'https://i.postimg.cc/FFdrCNqq/Untitled56-20260820115353.png') : '',

    // Granular Promo tags
    PROMO_CODE: rewardComp.promoCode,
    PROMO_POINTS: rewardComp.promoPoints,
    PROMO_SNIPPET_HTML: isRewardsCategoryVisible ? rewardComp.promoSnippetHtml : '',

    ESS1_NAME: dripData.ess1_name || 'Wooden Nametag',
    ESS1_IMG: getCdnImageUrl(dripData.ess1_img || 'https://lh3.googleusercontent.com/u/0/d/1F7Yb0OzuCmPO2LyZ0cMoaTM4d4rs5RFE', 'ess1.jpg'),
    ESS2_NAME: dripData.ess2_name || 'POS Kit',
    ESS2_IMG: getCdnImageUrl(dripData.ess2_img || 'https://lh3.googleusercontent.com/u/0/d/101jY71PjxCwiuNznTgn7Xyc0HoXwB3WQ', 'ess2.jpg'),
    GALLERY_URL: dripData.gallery_url || 'https://photos.app.goo.gl/6h7UPfkHU5TuvzXU7',
    HIGHLIGHT_DISPLAY: topProductsComp.display,
    HIGHLIGHT_ITEMS_HTML: topProductsComp.combinedHtml
  };

  for (let i = 1; i <= 9; i++) {
    const rawGImg = dripData[`grid${i}`] || defaultGrids[i - 1];
    replacements[`GRID${i}`] = getCdnImageUrl(rawGImg, `grid${i}.jpg`);
  }
...
export function renderOutOfWindowDripTemplate(dripData = {}, rewardProducts = [], activePromo = null) {
  ...
  const rawTemplate = (dripData.custom_html && dripData.custom_html.trim() !== '')
    ? dripData.custom_html
    : loadTemplateFile('out-of-window-drip.html');

  const templateTheme = extractTemplateTheme(rawTemplate);
  const rewardComp = buildRewardSectionComponents(points, rewardProducts, activePromo, templateTheme);
  const rewardSectionHtml = rewardComp.combinedHtml;
  ...
  return interpolatePlaceholders(rawTemplate, {
    ...
    // Granular tags for Custom Templates
    AFFORDABLE_REWARDS_HTML: rewardComp.affordableHtml,
    AFFORDABLE_COUNT: String(rewardComp.affordableCount),
    NEAREST_GOAL_HTML: rewardComp.goalHtml,
    NEAREST_GOAL_NAME: rewardComp.nearestItem ? rewardComp.nearestItem.name : '',
    NEAREST_GOAL_POINTS: rewardComp.nearestItem ? String(rewardComp.nearestItem.price) : '',
    NEAREST_GOAL_NEEDED: rewardComp.nearestItem ? String(rewardComp.needed) : '0',
    NEAREST_GOAL_IMAGE: rewardComp.nearestItem ? getCdnImageUrl(rewardComp.nearestItem.image_url || 'https://i.postimg.cc/FFdrCNqq/Untitled56-20260820115353.png') : '',
    NEAREST_GOAL_IMG: rewardComp.nearestItem ? getCdnImageUrl(rewardComp.nearestItem.image_url || 'https://i.postimg.cc/FFdrCNqq/Untitled56-20260820115353.png') : '',
    PROMO_CODE: rewardComp.promoCode,
    PROMO_POINTS: rewardComp.promoPoints,
    PROMO_SNIPPET_HTML: rewardComp.promoSnippetHtml
  });
}
...
export async function sendDripEmail(to, month = null, customName = "Elder Smith") {
  ...
  let promo = null;
  let rewardProducts = [];
  let dripData = { month: currentMonthNum, name: customName, points: currentMonthNum };

  try {
    if (to) {
      const mRows = await runSql("SELECT points, name FROM missionaries WHERE LOWER(email) = LOWER(?) LIMIT 1", [to]).catch(() => []);
      if (mRows && mRows.length > 0) {
        if (mRows[0].points !== undefined && mRows[0].points !== null) {
          dripData.points = Number(mRows[0].points);
        }
        if (customName === "Elder Smith" && mRows[0].name) {
          customName = mRows[0].name;
          dripData.name = customName;
        }
      }
    }
    ...
    const configRows = await runSql("SELECT value FROM system_settings WHERE key = ?", [`drip_${currentMonthNum}_highlight_meta`]);
    if (configRows && configRows.length > 0) {
      try {
        ...
        if (meta.drip_rewards_visible !== undefined) {
          dripData.show_rewards_category = Boolean(meta.drip_rewards_visible);
        }
      } catch (_) {}
    }

    const egRows = await runSql("SELECT value FROM system_settings WHERE key = ?", [`drip_${currentMonthNum}_essentials_grid`]).catch(() => []);
    if (egRows && egRows.length > 0) {
      try {
        const eg = JSON.parse(egRows[0].value);
        if (eg.ess1_name) dripData.ess1_name = eg.ess1_name;
        if (eg.ess1_img) dripData.ess1_img = eg.ess1_img;
        if (eg.ess2_name) dripData.ess2_name = eg.ess2_name;
        if (eg.ess2_img) dripData.ess2_img = eg.ess2_img;
        for (let i = 1; i <= 9; i++) {
          if (eg[`grid${i}`]) dripData[`grid${i}`] = eg[`grid${i}`];
        }
      } catch (_) {}
    }

    if (dripData.show_rewards_category === undefined) {
      const visibleRow = await runSql("SELECT value FROM system_settings WHERE key = 'drip_rewards_visible'").catch(() => []);
      if (visibleRow && visibleRow.length > 0) {
        dripData.show_rewards_category = visibleRow[0].value !== 'false';
      }
    }
  } catch (_) {}
