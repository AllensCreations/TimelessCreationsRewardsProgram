...
// Helper to map month index (1-24) to calendar name and cycle
export function getCalendarMonthLabel(monthIndex) {
  ...
  const cycleNum = Number(monthIndex) <= 12 ? "(Odd Year)" : "(Even Year)";
  return `${calendarNames[idx]} ${cycleNum}`;
}
...
export async function sendDripEmail(to, month = 1, customName = "Elder Smith") {
  ...
  return await sendEmail({
    to,
    subject: `Monthly Encouragement (${monthLabel}) • Timeless Creations`,
    htmlContent: html
  });
}
