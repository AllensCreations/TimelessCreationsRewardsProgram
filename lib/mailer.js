...
import { getCdnImageUrl } from './cdn.js';
 ...
 */
export async function sendEmail({ to, subject, htmlContent, month = 0, templateType = 'drip' }) {
  ...
  // 2. Inject invisible 1x1 open tracking pixel (Zero clicks needed, reads traffic on open)
  try {
    const rawDomain = (process.env.APP_URL || process.env.VERCEL_URL || '').trim();
    const appUrl = rawDomain ? (rawDomain.startsWith('http') ? rawDomain : `https://${rawDomain}`) : 'https://timelesscreationsrewardsprogram.vercel.app';
    const encodedEmail = Buffer.from((to || '').toLowerCase().trim()).toString('base64url');
    const pixelUrl = `${appUrl}/api/main?action=track_open&e=${encodedEmail}&m=${month || 0}&t=${encodeURIComponent(templateType || 'drip')}`;
    const pixelImgTag = `<!-- 1x1 Open Tracking Pixel --><img src="${pixelUrl}" width="1" height="1" alt="" style="display:none !important; width:1px; height:1px; border:0; opacity:0; pointer-events:none; visibility:hidden;" />`;

    if (finalHtml.includes('</body>')) {
      finalHtml = finalHtml.replace('</body>', `${pixelImgTag}\n</body>`);
    } else if (finalHtml.includes('</html>')) {
      finalHtml = finalHtml.replace('</html>', `${pixelImgTag}\n</html>`);
    } else {
      finalHtml += pixelImgTag;
    }
  } catch (_) {}
...
export function buildTopProductsHtml(dripData = {}) {
  ...
  const p1Img = getCdnImageUrl(dripData.highlight_img || "https://lh3.googleusercontent.com/u/0/d/1F7Yb0OzuCmPO2LyZ0cMoaTM4d4rs5RFE", "product_1.jpg");
  const p1Sold = formatMetricK(dripData.highlight_sold_1 || "1.5k+ Sold");

  const p2Name = (dripData.highlight_label_2 || "").trim();
  const p2Img = getCdnImageUrl(dripData.highlight_img_2 || "https://lh3.googleusercontent.com/u/0/d/101jY71PjxCwiuNznTgn7Xyc0HoXwB3WQ", "product_2.jpg");
  const p2Sold = formatMetricK(dripData.highlight_sold_2 || "950+ Sold");
...
export function buildCombinedRewardSectionHtml(userPoints = 2, catalogProducts = [], activePromo = null) {
  ...
  if (validProducts.length > 0) {
    ...
    const nearestItem = sorted.find(p => Number(p.price) > pts) || sorted[0];
    const needed = Math.max(1, (Number(nearestItem.price) || 6) - pts);
...
export function renderMonthlyDripTemplate(dripData = {}, rewardProducts = [], activePromo = null) {
  ...
  const rewardSectionHtml = isRewardsCategoryVisible ? buildCombinedRewardSectionHtml(points, rewardProducts, activePromo) : '';

  const defaultGrids = [
    "https://cdn.jsdelivr.net/gh/AllensCreations/TimelessCreationsRewardsProgram@Appversion/public/drips/grid_1.jpg",
    "https://cdn.jsdelivr.net/gh/AllensCreations/TimelessCreationsRewardsProgram@Appversion/public/drips/grid_2.jpg",
    "https://cdn.jsdelivr.net/gh/AllensCreations/TimelessCreationsRewardsProgram@Appversion/public/drips/grid_3.jpg",
    "https://cdn.jsdelivr.net/gh/AllensCreations/TimelessCreationsRewardsProgram@Appversion/public/drips/grid_4.jpg",
    "https://cdn.jsdelivr.net/gh/AllensCreations/TimelessCreationsRewardsProgram@Appversion/public/drips/grid_5.jpg",
    "https://cdn.jsdelivr.net/gh/AllensCreations/TimelessCreationsRewardsProgram@Appversion/public/drips/grid_6.jpg",
    "https://cdn.jsdelivr.net/gh/AllensCreations/TimelessCreationsRewardsProgram@Appversion/public/drips/grid_7.jpg",
    "https://cdn.jsdelivr.net/gh/AllensCreations/TimelessCreationsRewardsProgram@Appversion/public/drips/grid_8.jpg",
    "https://cdn.jsdelivr.net/gh/AllensCreations/TimelessCreationsRewardsProgram@Appversion/public/drips/grid_9.jpg"
  ];
...
export async function sendDripEmail(to, month = null, customName = "Elder Smith") {
  ...
  return await sendEmail({ to, subject: subjectLine, htmlContent: html, month: targetMonthNumber, templateType: 'drip' });
}
...
export async function sendOTPEmail(to, otpCode = "749281", customName = "Missionary") {
  ...
  return await sendEmail({ to, subject: `🔐 Your TCRP Verification Code: ${otpCode}`, htmlContent: html, month: 0, templateType: 'otp' });
}
...
export async function sendReceiptEmail(to, order = { name: "Elder / Sister", order_id: "TCRP-9921", item: "Wooden Missionary Nametag", points_cost: 6 }) {
  ...
  return await sendEmail({ to, subject: `🧾 Redemption Receipt (${order.order_id || 'TCRP-9921'}) • Timeless Creations`, htmlContent: html, month: 0, templateType: 'receipt' });
}
...
export async function sendThankYouEmail(to, order = { name: "Elder / Sister", order_id: "TCRP-9921", item: "Wooden Missionary Nametag" }) {
  ...
  return await sendEmail({ to, subject: `📦 Order Completed (${order.order_id || 'TCRP-9921'}) • Timeless Creations`, htmlContent: html, month: 0, templateType: 'thankyou' });
}
...
export async function sendOrderStatusEmail(to, order = { name: "Elder / Sister", order_id: "TCRP-0000", item: "Item" }, status = "PAID") {
  ...
  return await sendEmail({ to, subject: `📦 Order Update [${status}] • ${order.order_id || 'TCRP-0000'}`, htmlContent: html, month: 0, templateType: 'order_status' });
}
...
export async function sendDeliveredEmail(to, order = { name: "Elder / Sister", order_id: "TCRP-0000", item: "Item" }) {
  ...
  return await sendEmail({ to, subject: `📦 Order Delivered (${order.order_id || 'TCRP-0000'}) • Timeless Creations`, htmlContent: html, month: 0, templateType: 'delivered' });
}
