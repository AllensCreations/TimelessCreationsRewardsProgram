...
export function renderMonthlyDripTemplate(dripData = {}) {
  const templatePath = path.resolve("templates/monthly-drip.html");
  let template = fs.readFileSync(templatePath, "utf8");

  const month = dripData.month || 1;
  ...
  const message = (dripData.message || `May your faith be strengthened as you serve and invite others to come unto Christ during Month ${month}.`).replace(/\n/g, '<br>');
  const points = dripData.points !== undefined ? dripData.points : 2;

  // Max 2 Highlights Table Generator
  let highlightHtml = '';
  const highlights = [];
  ...
  if (hasHighlights) {
    ...
    highlightHtml = highlights.map(h => `
      <td align="center" width="${colWidth}" valign="top" style="padding:4px;">
        <img src="${h.img}" style="width:100%; max-width:130px; aspect-ratio:1/1; object-fit:cover; border-radius:4px; border:1px solid #c9a84c; display:block; margin:0 auto 6px auto;" alt="Highlight">
...
export async function sendDripEmail(to, month = 1, customName = "Elder Smith") {
  ...
  const html = renderMonthlyDripTemplate(dripData);
