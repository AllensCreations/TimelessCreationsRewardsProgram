  ...
  if (p1Name && !p2Name) {
    return `
      <div style="margin: 20px 0; padding: 16px 12px; background: #faf7f0; border: 1.5px solid #c9a84c; border-radius: 6px; text-align: center;">
        ...
        <div style="font-size: 10px; color: #8c7e5d; font-weight: bold; margin-top: 2px;">🔥 ${formatMetricK(p1Sold)}</div>
      </div>
    `;
  }
  ...
  if (!p1Name && p2Name) {
    return `
      <div style="margin: 20px 0; padding: 16px 12px; background: #faf7f0; border: 1.5px solid #c9a84c; border-radius: 6px; text-align: center;">
        ...
        <img src="${p2Img}" style="width: 100%; max-width: 130px; aspect-ratio: 1/1; object-fit: cover; border-radius: 4px; border: 1.5px solid #c9a84c; display: block; margin: 0 auto 6px auto;" alt="${p2Name}">
        <div style="font-family: 'Helvetica', Arial, sans-serif; font-size: 11.5px; font-weight: bold; color: #1a1a1a;">${p2Name}</div>
        <div style="font-size: 10px; color: #8c7e5d; font-weight: bold; margin-top: 2px;">🔥 ${formatMetricK(p2Sold)}</div>
      </div>
    `;
  }
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0" style="table-layout: fixed;">
        <tr>
          ...
          <td align="center" width="48%" valign="top" style="background: #ffffff; border: 1.5px solid #c9a84c; border-radius: 6px; padding: 12px 8px; box-shadow: 0 4px 10px rgba(201,168,76,0.12);">
            ...
            <div style="font-size: 9.5px; color: #8c7e5d; font-weight: bold; margin-top: 3px;">🔥 ${formatMetricK(p1Sold)}</div>
          </td>
          <td width="4%"></td>
          <!-- TOP 2 (Column 2) -->
          <td align="center" width="48%" valign="top" style="background: #fdfdfd; border: 1px dashed #d4c197; border-radius: 6px; padding: 12px 8px;">
            ...
            <img src="${p2Img}" style="width: 100%; max-width: 95px; aspect-ratio: 1/1; object-fit: cover; border-radius: 4px; border: 1px solid #d4c197; display: block;" alt="${p2Name}">
            ...
            <div style="font-size: 9px; color: #8c7e5d; font-weight: bold; margin-top: 3px;">⭐ ${formatMetricK(p2Sold)}</div>
          </td>
        </tr>
      </table>
    </div>
 ...
 */
export function buildTopProductsComponents(dripData = {}) {
  const isExplicitlyDisabled = dripData.highlight_enabled === false || (dripData.highlight_label || "").trim() === "__DISABLED__";
  ...
  const combinedHtml = buildTopProductsHtml(dripData);
 ...
 */
export function buildRewardSectionComponents(userPoints = 2, catalogProducts = [], activePromo = null) {
  ...
  if (activePromo && activePromo.code && activePromo.code.trim() !== "") {
    ...
    promoSnippetHtml = `
      <div style="margin: 0 0 14px 0; padding: 12px 14px; background-color: #fff9e6; border: 1.5px dashed #c9a84c; border-radius: 6px; text-align: center;">
        ...
        <div style="font-family: monospace; font-size: 13px; font-weight: bold; background: #1a1a1a; color: #d4c197; padding: 6px 14px; border-radius: 4px; display: inline-block; letter-spacing: 1px;">
          ${promoCode}
        </div>
              ...
              <table width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr>
                  <td width="36" valign="middle">
                    <img src="${getCdnImageUrl(item.image_url || 'https://i.postimg.cc/FFdrCNqq/Untitled56-20260820115353.png')}" style="width: 32px; height: 32px; aspect-ratio: 1/1; object-fit: cover; border-radius: 3px; display: block;">
                  </td>
  ...
  if (unreached.length > 0) {
    ...
    goalHtml = `
      <div style="background: #ffffff; border: 1px solid #e0d6bc; border-radius: 6px; padding: 12px; text-align: center; margin-bottom: 10px;">
        ...
        <img src="${getCdnImageUrl(nearestItem.image_url || 'https://i.postimg.cc/FFdrCNqq/Untitled56-20260820115353.png')}" style="width: 50px; height: 50px; aspect-ratio: 1/1; object-fit: cover; border-radius: 4px; display: block; margin: 0 auto 6px auto; border: 1px solid #d4c197;">
...
export function buildCombinedRewardSectionHtml(userPoints = 2, catalogProducts = [], activePromo = null) {
  return buildRewardSectionComponents(userPoints, catalogProducts, activePromo).combinedHtml;
}
...
export function renderMonthlyDripTemplate(dripData = {}, rewardProducts = [], activePromo = null) {
  ...
  const topProductsComp = buildTopProductsComponents(dripData);
  ...
  const rewardComp = buildRewardSectionComponents(points, rewardProducts, activePromo);
  ...
  const replacements = {
    ...
    // Unified Macro Blocks (backward-compatible)
  ...
  for (let i = 1; i <= 9; i++) {
    replacements[`GRID${i}`] = getCdnImageUrl(dripData[`grid${i}`] || defaultGrids[i - 1], `grid${i}.jpg`);
  }
