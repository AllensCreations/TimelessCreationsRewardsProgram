...
export function buildTopProductsHtml(dripData = {}) {
  ...
  const p1Sold = dripData.highlight_sold_1 || "150+ Sold";
  ...
  const p2Sold = dripData.highlight_sold_2 || "90+ Sold";

  if (!p1Name && !p2Name) return "";

  if (p1Name && !p2Name) {
    return `
      <div style="margin: 20px 0; padding: 18px 14px; background: #faf7f0; border: 1.5px solid #c9a84c; border-radius: 6px; text-align: center;">
        <span style="display: inline-block; background: #8b1a1a; color: #ffffff; font-family: 'Helvetica', Arial, sans-serif; font-size: 9px; letter-spacing: 2px; text-transform: uppercase; font-weight: bold; padding: 4px 10px; border-radius: 4px; margin-bottom: 10px;">
          🥇 #1 TOP BESTSELLER
        </span>
        <img src="${p1Img}" style="width: 100%; max-width: 155px; aspect-ratio: 1/1; object-fit: cover; border-radius: 6px; border: 2px solid #c9a84c; display: block; margin: 0 auto 8px auto;" alt="${p1Name}">
        <div style="font-family: 'Helvetica', Arial, sans-serif; font-size: 12px; font-weight: bold; color: #1a1a1a;">${p1Name}</div>
        <div style="font-size: 10px; color: #8c7e5d; font-weight: bold; margin-top: 3px;">🔥 ${p1Sold}</div>
      </div>
    `;
  }

  return `
    <div style="margin: 20px 0; padding: 18px 14px; background: #faf7f0; border: 1.5px solid #c9a84c; border-radius: 6px;">
      <div style="font-family: 'Helvetica', Arial, sans-serif; font-size: 9px; letter-spacing: 2px; text-transform: uppercase; color: #8b1a1a; font-weight: bold; text-align: center; margin-bottom: 14px;">
        ⭐ Top Products of the Month
      </div>
      
      <!-- TOP 1 (Larger Card) -->
      <div style="background: #ffffff; border: 1.5px solid #c9a84c; border-radius: 6px; padding: 14px 10px; text-align: center; margin-bottom: 12px; box-shadow: 0 4px 12px rgba(201,168,76,0.15);">
        ...
        <img src="${p1Img}" style="width: 100%; max-width: 145px; aspect-ratio: 1/1; object-fit: cover; border-radius: 4px; border: 1.5px solid #c9a84c; display: block; margin: 0 auto 6px auto;" alt="${p1Name}">
        <div style="font-family: 'Helvetica', Arial, sans-serif; font-size: 12px; font-weight: bold; color: #1a1a1a;">${p1Name}</div>
        <div style="font-size: 10px; color: #8c7e5d; font-weight: bold; margin-top: 2px;">🔥 ${p1Sold}</div>
      </div>

      <!-- TOP 2 (Runner-up Card) -->
      <div style="background: #fdfdfd; border: 1px dashed #d4c197; border-radius: 6px; padding: 10px; text-align: center;">
        <span style="display: inline-block; background: #555566; color: #ffffff; font-family: 'Helvetica', Arial, sans-serif; font-size: 7.5px; letter-spacing: 1.5px; text-transform: uppercase; font-weight: bold; padding: 2px 6px; border-radius: 3px; margin-bottom: 6px;">
          🥈 TOP 2 POPULAR
        </span>
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td width="90" align="center" valign="middle">
              <img src="${p2Img}" style="width: 80px; height: 80px; aspect-ratio: 1/1; object-fit: cover; border-radius: 4px; border: 1px solid #d4c197; display: block;" alt="${p2Name}">
            </td>
            <td style="padding-left: 10px; text-align: left;" valign="middle">
              <div style="font-family: 'Helvetica', Arial, sans-serif; font-size: 11px; font-weight: bold; color: #1a1a1a;">${p2Name}</div>
              <div style="font-size: 9.5px; color: #8c7e5d; font-weight: bold; margin-top: 3px;">⭐ ${p2Sold}</div>
            </td>
          </tr>
        </table>
      </div>
