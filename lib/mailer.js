...
export function buildRewardSectionHtml(points = 2, products = []) {
  ...
  if (affordable.length > 0) {
    ...
    whatCanIGetHtml = `
      <div style="font-size:11px; color:#666; font-style:italic; margin-bottom:10px; text-align:left;">
        🎁 <strong>What You Can Get:</strong> Keep earning points through monthly dispatches and referrals to unlock claimable rewards!
      </div>
    `;
  }
...
export function renderMonthlyDripTemplate(dripData = {}, rewardProducts = []) {
  ...
  const masterTemplate = fs.existsSync(masterTemplatePath) ? fs.readFileSync(masterTemplatePath, "utf8") : "";
...
export async function sendDripEmail(to, month = 1, customName = "Elder Smith") {
  ...
  try {
    ...
    if (dbDrip) dripData = { ...dripData, ...dbDrip };
    ...
    if (missionary && missionary.points !== undefined) dripData.points = missionary.points;
  ...
  const subjectLine = dripData.subject && dripData.subject.trim() !== "" ? dripData.subject : `Monthly Encouragement (${monthLabel}) • Timeless Creations`;
...
export async function sendOTPEmail(to, otpCode = "749281") {
  ...
  let html = fs.existsSync(templatePath) ? fs.readFileSync(templatePath, "utf8").replace(/{{OTP_CODE}}/g, otpCode) : `<h2>Verification Code: ${otpCode}</h2>`;
...
export async function sendReceiptEmail(to, order = { name: "Elder / Sister", order_id: "TCRP-9921", item: "Wooden Missionary Nametag", points_cost: 6 }) {
  ...
  let html = fs.existsSync(templatePath) ? fs.readFileSync(templatePath, "utf8")
    .replace(/{{NAME}}/g, order.name || "Missionary")
    .replace(/{{ORDER_ID}}/g, order.order_id || "TCRP-ORDER")
    .replace(/{{ITEM}}/g, order.item || "Reward Item")
    .replace(/{{POINTS_COST}}/g, String(order.points_cost || 0)) : `<h2>Redemption Receipt: ${order.order_id}</h2>`;
...
export async function sendThankYouEmail(to, order = { name: "Elder / Sister", order_id: "TCRP-9921", item: "Wooden Missionary Nametag" }) {
  ...
  let html = fs.existsSync(templatePath) ? fs.readFileSync(templatePath, "utf8")
    .replace(/{{NAME}}/g, order.name || "Missionary")
    .replace(/{{ORDER_ID}}/g, order.order_id || "TCRP-ORDER")
    .replace(/{{ITEM}}/g, order.item || "Reward Item") : `<h2>Order Completed: ${order.order_id}</h2>`;
...
export async function sendOrderStatusEmail(to, order = { name: "Missionary", order_id: "TCRP-0000", item: "Item" }, status = "PAID") {
  if (status === "DELIVERED") {
    const templatePath = path.resolve("templates/delivered-email.html");
    if (fs.existsSync(templatePath)) {
      const html = fs.readFileSync(templatePath, "utf8")
        .replace(/{{NAME}}/g, order.name || "Customer")
        .replace(/{{ORDER_ID}}/g, order.order_id || "ORDER")
        .replace(/{{ITEM}}/g, order.item || "Reward / Product");
      return await sendEmail({ to, subject: `🎉 Order Delivered • Thank You! (${order.order_id}) • Timeless Creations`, htmlContent: html });
    }
  }
  const html = `<div style="font-family:Georgia,serif; max-width:450px; margin:0 auto; background:#ffffff; border:1px solid #e0d6bc; padding:30px; color:#1a1a1a;"><h2 style="color:#8b1a1a;">Timeless Creations Update</h2><p>Hello <strong>${order.name}</strong>,</p><p>Status for order <strong>${order.order_id}</strong> updated to: <strong style="color:#8b1a1a;">${status}</strong></p></div>`;
