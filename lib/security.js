...
export async function checkDailyViewLimit(senderId, dailyMax = 1) {
  ...
  try {
    const rows = await runSql(
      "SELECT view_count, warned FROM bot_daily_views WHERE sender_id = ? AND view_date = ? LIMIT 1",
      [sid, todayStr]
    );

    const currentCount = Number(rows?.[0]?.view_count || 0);
    const warned = Number(rows?.[0]?.warned || 0);
...
export async function hasUsedDailyCheck(senderId) {
  ...
  try {
    ...
    const count = Number(rows?.[0]?.view_count || 0);
    return count >= 1;
  } catch (_) {
    return false;
  }
