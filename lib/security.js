...
/**
 * Returns today's quota tracking date, resetting every morning at 8:00 AM PHT (00:00 UTC).
 * Format: YYYY-MM-DD
 */
...
export function clearDebounce(psid = null, clearViews = true) {
  if (!psid) {
    ...
    if (clearViews) runSql("DELETE FROM bot_limits").catch(() => {});
    return;
  }
  for (const [k] of recentDebounceMap.entries()) {
    if (k.startsWith(`${psid}:`)) recentDebounceMap.delete(k);
  }
  if (clearViews) runSql("DELETE FROM bot_limits WHERE psid = ?", [psid]).catch(() => {});
}
...
export async function checkBurstRateLimit(psid, limit = 12, windowSeconds = 60) {
  ...
  const rows = await runSql("SELECT burst_count, burst_window_start, burst_warned FROM bot_limits WHERE psid = ? AND limit_date = ? LIMIT 1", [psid, todayStr]);
  const record = rows?.[0];

  if (!record) {
    await runSql(
      "INSERT INTO bot_limits (psid, limit_date, burst_count, burst_window_start, burst_warned) VALUES (?, ?, 1, ?, 0) ON CONFLICT(psid, limit_date) DO UPDATE SET burst_count = 1, burst_window_start = ?, burst_warned = 0",
      [psid, todayStr, now, now]
    );
    return { limited: false, silentDrop: false };
  }

  if (now - Number(record.burst_window_start) > windowSeconds) {
    await runSql("UPDATE bot_limits SET burst_count = 1, burst_window_start = ?, burst_warned = 0 WHERE psid = ? AND limit_date = ?", [now, psid, todayStr]);
    return { limited: false, silentDrop: false };
  }

  if (Number(record.burst_count) >= limit) {
    const alreadyWarned = Number(record.burst_warned) === 1;
    if (!alreadyWarned) {
      await runSql("UPDATE bot_limits SET burst_warned = 1 WHERE psid = ? AND limit_date = ?", [psid, todayStr]);
      return { limited: true, silentDrop: false };
    }
    return { limited: true, silentDrop: true };
  }

  await runSql("UPDATE bot_limits SET burst_count = burst_count + 1 WHERE psid = ? AND limit_date = ?", [psid, todayStr]);
  return { limited: false, silentDrop: false };
}
...
export async function checkDailyMessageQuota(psid, isVerified = false) {
  ...
  try {
    const rows = await runSql(
      "SELECT daily_msg_count, daily_msg_warned, otp_resend_count FROM bot_limits WHERE psid = ? AND limit_date = ? LIMIT 1",
      [psid, todayStr]
    );
    const record = rows?.[0];
    ...
    if (!record) {
      await runSql(
        "INSERT INTO bot_limits (psid, limit_date, daily_msg_count, daily_msg_warned, otp_resend_count) VALUES (?, ?, 1, 0, 0) ON CONFLICT(psid, limit_date) DO UPDATE SET daily_msg_count = 1",
        [psid, todayStr]
      );
      return { allowed: true, current: 1, limit, remaining: Math.max(0, limit - 1) };
    }

    const currentCount = Number(record.daily_msg_count) || 0;
    if (currentCount >= limit) {
      const alreadyWarned = Number(record.daily_msg_warned) === 1;
      if (!alreadyWarned) {
        await runSql("UPDATE bot_limits SET daily_msg_warned = 1 WHERE psid = ? AND limit_date = ?", [psid, todayStr]);
    ...
    await runSql("UPDATE bot_limits SET daily_msg_count = ? WHERE psid = ? AND limit_date = ?", [newCount, psid, todayStr]);
...
export async function checkOtpResendEligibility(psid, session) {
  ...
  try {
    const rows = await runSql(
      "SELECT otp_resend_count FROM bot_limits WHERE psid = ? AND limit_date = ? LIMIT 1",
      [psid, todayStr]
    );
...
export async function recordOtpResend(psid) {
  ...
  try {
    ...
    if (!rows || rows.length === 0) {
      await runSql(
        "INSERT INTO bot_limits (psid, limit_date, daily_msg_count, daily_msg_warned, otp_resend_count) VALUES (?, ?, 0, 0, 1) ON CONFLICT(psid, limit_date) DO UPDATE SET otp_resend_count = 1",
        [psid, todayStr]
      );
      return 1;
    }
    ...
    await runSql(
      "UPDATE bot_limits SET otp_resend_count = ? WHERE psid = ? AND limit_date = ?",
...
export async function checkDailyViewLimit(senderId, dailyMax = 1) {
  ...
  try {
    const rows = await runSql(
      "SELECT daily_view_count, daily_view_warned FROM bot_limits WHERE psid = ? AND limit_date = ? LIMIT 1",
      [sid, todayStr]
    );

    const currentCount = Number(rows?.[0]?.daily_view_count || 0);
    const warned = Number(rows?.[0]?.daily_view_warned || 0);

    if (currentCount >= dailyMax) {
      const shouldWarn = (warned === 0);
      if (shouldWarn) {
        await runSql(
          "UPDATE bot_limits SET daily_view_warned = 1 WHERE psid = ? AND limit_date = ?",
          [sid, todayStr]
        ).catch(() => {});
      }
      return { allowed: false, remaining: 0, current: currentCount, dailyMax, shouldWarn };
    }

    const newCount = currentCount + 1;
    await runSql(`
      INSERT INTO bot_limits (psid, limit_date, daily_view_count, daily_view_warned) 
      VALUES (?, ?, ?, 0) 
      ON CONFLICT(psid, limit_date) DO UPDATE SET daily_view_count = ?
    `, [sid, todayStr, newCount, newCount]);

    return { allowed: true, remaining: dailyMax - newCount, current: newCount, dailyMax, shouldWarn: false };
  } catch (err) {
    return { allowed: true, remaining: 1, current: 1, dailyMax, shouldWarn: false };
  }
}

/**
 * Read-only check whether the user has already consumed their 1 check today.
 * Resets daily at 8:00 AM PHT (00:00 UTC).
 */
export async function hasUsedDailyCheck(senderId) {
  const sid = String(senderId);
  const todayStr = getDailyViewDateStr();
  try {
    const rows = await runSql(
      "SELECT daily_view_count FROM bot_limits WHERE psid = ? AND limit_date = ? LIMIT 1",
      [sid, todayStr]
    );
    const count = Number(rows?.[0]?.daily_view_count || 0);
    return count >= 1;
  } catch (_) {
    return false;
  }
