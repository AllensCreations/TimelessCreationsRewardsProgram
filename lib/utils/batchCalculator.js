...
/**
 * Returns the estimated 1st dispatch month (YYYY-MM) for a missionary's batch cohort.
 * There is NO day preset, allowing dispatch on any operational day during that month.
 * e.g., if batch is September 2026, returns "2026-10".
 *
 * @param {string} batchMonthStr
 * @param {Date} baseDate
 * @returns {string} Month string YYYY-MM (no day preset)
 */
 ...
 * Fast-forwards skipped months so the template sent is ALWAYS the current calendar month.
 *
 * @param {string} batchMonthStr
 * @param {number} maxMonths
 * @param {Date} targetDate
 * @returns {object} { targetCalMonth, calYear, calMonthName, calMonthYear, curMissionMonth, tenureLabel, displayLabel }
 */
export function getDripForTargetDate(batchMonthStr, maxMonths = 24, targetDate = new Date()) {
 ...
 * Synchronizes with the current calendar month:
 * - If missionary is behind (months_sent < curMissionMonth), upcoming is the CURRENT calendar month (skipping past missed months).
 * - If missionary is in Month 0 (arrival month), upcoming is Month 1 (next month).
 * - If missionary has already received current month (months_sent >= curMissionMonth), upcoming is next month (curMissionMonth + 1).
 *
 * @param {string} batchMonthStr
 * @param {number} monthsSent
 * @param {number} maxMonths
 * @param {Date} targetDate
 * @returns {object|null} { monthNum, year, monthName, display, tenureMonth, displayLabel }
 */
export function getUpcomingDripInfo(batchMonthStr, monthsSent = 0, maxMonths = 24, targetDate = new Date()) {
  ...
  if (curMissionMonth <= 0) {
    // Arrival month (Month 0). Next drip is Month 1.
    ...
    // Missed past months or due for current month. Send CURRENT calendar month at milestone M{curMissionMonth}.
    targetCalMonth = calMonthNum;
    targetYear = calYear;
    tenureMonth = curMissionMonth;
  } else {
    // Already received current month (or ahead). Next drip is next month.
  ...
  const calMonthName = MONTH_NAMES[targetCalMonth - 1];
...
export function isMissionaryEligibleForDispatch(missionary, targetDate = new Date(), todayIso = null) {
  ...
  // If already dispatched in current calendar month (YYYY-MM), NOT due again this month (1 drip per calendar month)
  if (missionary.last_sent_at && missionary.last_sent_at.slice(0, 7) === todayStr.slice(0, 7)) return false;
  // Compare month (YYYY-MM) so there is NO day preset blocking dispatch during the due month
  if (missionary.next_send_date && missionary.next_send_date.slice(0, 7) > todayStr.slice(0, 7)) return false;
