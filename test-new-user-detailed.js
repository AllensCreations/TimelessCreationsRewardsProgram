...
async function runDetailedLoggerTest() {
  console.log("🔍 [DIAGNOSTIC LOGGER] Starting Detailed New User Verification (Type-Safe)...\n");
  let passed = 0;
  let failed = 0;
  ...
  try {
    ...
    // Coerce to Number for type-safe assertion
    assert(Number(userRows[0]?.points) === 0, `Expected points to be 0, got: ${userRows[0]?.points}`, userRows[0]);
    assert(userRows[0]?.referral_code === testReferralCode, "Referral code match check");
    ...
    // Coerce to Number for type-safe assertion
    assert(Number(updatedUserRows[0]?.points) === 1, `Expected points to increment to 1, got: ${updatedUserRows[0]?.points}`, updatedUserRows[0]);
    ...
    // -------------------------------------------------------------------------
    const currentPoints = Number(updatedUserRows[0]?.points) || 1;
    ...
    const snippet = renderedDrip.slice(pointsSnippetIndex, pointsSnippetIndex + 200);

    assert(renderedDrip.includes("Elder Detailed"), "Template binds missionary name");
    assert(renderedDrip.includes(`<strong>${currentPoints}</strong> Points`), `Template reflects points value (${currentPoints} Points)`, { snippet });
