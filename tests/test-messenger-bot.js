...
async function runFullBotTester() {
  ...
  try {
    ...
    await runSql("DELETE FROM system_logs WHERE source = 'CHAT_MESSENGER' AND psid = ?", [testPsid]);
    ...
    const rejectMsg = (await runSql("SELECT message FROM system_logs WHERE source = 'CHAT_MESSENGER' AND psid = ? AND sender = 'bot' ORDER BY id DESC LIMIT 1", [testPsid]))[0];
    ...
    const msgs = await runSql("SELECT message FROM system_logs WHERE source = 'CHAT_MESSENGER' AND psid = ? AND sender = 'bot' ORDER BY id DESC LIMIT 1", [testPsid]);
